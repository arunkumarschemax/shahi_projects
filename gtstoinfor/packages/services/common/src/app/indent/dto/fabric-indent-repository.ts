...
export class FabricIndentRepository extends Repository<IndentFabricEntity> {
    ...
    async getFabricIndentData(indentId: number) {
        ...
        co.colour,it.status, uom.uom AS quantityUnit,CONCAT(b.buyer_code,'-',b.buyer_name)AS buyer,s.buyer_id AS buyerId, it.style as styleId,IF(sum(po_quantity) IS null, 0,sum(po_quantity)) as poQty,IF(sum(po_quantity) IS null, 0,sum(po_quantity)) as poQuantity,itf.season,itf.xl_no as xlNo, (itf.quantity - IF(sum(po_quantity) IS null, 0,sum(po_quantity))) AS toBeProcured,m3.hsn_code AS hsnCode`)
