...
export class IndentService{
    ...
    async getAllIndentItemDetailsAgainstIndent(req:indentIdReq):Promise<CommonResponseModel>{
        ...
        const data='SELECT "" AS poQuantity,ifb.ifabric_id AS indentFabricId,ifb.indent_id AS indentid,   m3_style_code AS m3FabricCode,colour AS colorName,m3_fabric_code m3FabricCodeId,color AS colourId,quantity AS indentQuantity,ifb.indent_id AS indentId FROM indent_fabric ifb LEFT JOIN indent i ON ifb.indent_id=i.indent_id LEFT JOIN colour c ON c.colour_id=ifb.color LEFT JOIN m3_style ms ON ms.m3_style_Id=ifb.m3_fabric_code WHERE ifb.indent_id IN ('+req.indentId+')'
        const result= await this.indentRepo.query(data)
        if(result){
            return new CommonResponseModel(true,1,'data retived sucessfully',result)
        }else{
