...
export class IndentService {
        ...
        for (const data of indentData) {
            ...
            for (const fabric of fabricIndentData) {
                fabricModel.push(new IndentFabricModel(fabric.ifabric_id, fabric.content,
                    ...
                    fabric.buyer_name, fabric.grn_date, fabric.xl_no, fabric.quantity, fabric.quantityUnit, fabric.status,fabric.indentId,fabric.materialType))
            }
    ...
    async getAllIndentTrimDetailsAgainstIndent(req: indentIdReq): Promise<CommonResponseModel> {
        ...
        const data ='SELECT m3_trim_Id as m3TrimCode,request_no as indentCode,"" AS poQuantity,ms.trim_type AS trimtype,ms.trim_code AS m3TrimCodeName,it.trim_type AS productGroupId,it.trim_code AS trimId,  c.colour AS colourName,itrims_id AS indentTrmId, it.trim_code AS trimId ,size AS sizeId,color AS colourId, quantity AS indentQuantity,quantity_unit AS indentQuantityUnit FROM indent_trims it    LEFT JOIN indent i ON it.indent_id=i.indent_id          LEFT JOIN colour c ON c.colour_id=it.color  LEFT JOIN m3_trims ms ON ms.m3_trim_Id=it.trim_code where it.indent_id in(' + req.indentId + ') '
        const result = await this.indentRepo.query(data)
        if (result) {
            return new CommonResponseModel(true, 1, 'data retived sucessfully', result)
        } else {
