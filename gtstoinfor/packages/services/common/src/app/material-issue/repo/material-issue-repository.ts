...
export class MaterialIssueRepository extends Repository<MaterialIssueEntity> {
    ...
    async getMaterialIssue (){
        ...
        .select(`mi.material_issue_id as id,mi.consumption_code as consumptioncode,mi.request_no as requestNo,mi.issue_date as date,mi.location_id as locationId ,mi.pch_id as pchId ,mi.buyer_id as buyer ,mi.sample_type_id as sampletype,mi. style_no as style,mi.brand_id ,mi.dmm_id,mi.technician_id,mi.description,mi.cost_ref,mi.m3_style_no as styleno,mi.contact,mi.extn,mi.extn,mi.product,mi.type,mi.conversion,mi.made_in ,fb.material_fabric_id,fb.fabric_code as fabricCode,fb.description as fbdescription,fb.color_id as colour,fb.consumption as consumption,fb.consumption_uom,fb.issued_quantity as issuedQuantity,fb.issued_quantity_uom,tr.material_trim_id,tr.description,tr.color_id,tr.consumption as consumption,tr.consumption_uom,tr.issued_quantity as issuedQuantity,tr.issued_quantity_uom,c.colour_id as colorId`)
        ...
        .leftJoin(Colour,'c','c.colour_id = c.colour_id')
        .groupBy('mi.material_issue_id')
