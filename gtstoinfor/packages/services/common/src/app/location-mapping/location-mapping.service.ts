...
export class LocationMappingService {
    ...
    async getAllFabrics(req?:ExternalRefReq): Promise<CommonResponseModel> {
        try {
            ...
            IF(g.item_type = "FABRIC", mit.description, mtr.trim_code) AS itemCode, g.grn_number AS grnNumber, v.vendor_name, gi.accepted_quantity AS acceptedQuantity, gi.buyer_id AS buyerId, idfb.buyer_name AS buyerName,g.grn_id as grnId, g.grn_type AS grnType, gi.item_type AS itemType,mit.description
