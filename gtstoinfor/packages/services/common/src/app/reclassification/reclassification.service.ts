...
async ApproveStockReclassification(req:any): Promise<CommonResponseModel> {
  const manager=this.dataSource
    try {
      ...
      if(updateStatus.affected > 0){
        const stocksEntity = new StocksEntity()
          ...
          let saveStock = await manager.getRepository(StocksEntity).save(stocksEntity)
          if(saveStock.id > 0){
            let updateStockQty = await manager.getRepository(StocksEntity).update({id:req.stockId},{ quantity: () => `quantity-${req.quantity}`});
            if(updateStockQty.affected > 0){
              // await manager.completeTransaction();
