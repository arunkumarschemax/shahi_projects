...
export class ItemCreationRepository extends Repository<ItemCreation> {
    ...
    async getAllFgItemCrted(req: ItemCreFilterRequest): Promise<any[]> {
        ...
        .select(`fg_item_id,fgi.is_active,reference,style_no,item_name, item_type, item_code, brand_name, item_category, item_sub_category,CONCAT(ed.first_name, ' ', ed.last_name) AS responsible_person,CONCAT(epr.first_name, ' ', epr.last_name) AS product_designer,
