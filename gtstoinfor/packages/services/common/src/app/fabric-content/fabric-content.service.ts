...
export class FabricContentService {
  ...
  async uploadFabricContent(formData: any): Promise<CommonResponseModel> {
    ...
    try {
      ...
      const updatedArray = formData.map((obj) => {
        ...
        for (const key in obj) {
          const newKey = key.replace(/\s/g, '_').replace(/[\(\)]/g, '').replace(/#/g, '');
      ...
      for (const data of convertedData) {
        ...
        if (data.STYLE !== null) {
          ...
          const dtoData = new FabricContentDto()
          dtoData.style = data.STYLE
          dtoData.component = data.Component_
          dtoData.fabricContent = data.Fabric_Content
