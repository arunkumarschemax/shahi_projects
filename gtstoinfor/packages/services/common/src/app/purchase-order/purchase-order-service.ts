...
async cretePurchaseOrder(req:PurchaseOrderDto):Promise<CommonResponseModel>{
    ...
    async getAllPoData ():Promise<CommonResponseModel>{
        try{
            ...
            pot.consumption,pot.remarks as trimRemarks,pot.product_group_id AS trimProductGroupId,tpg.product_group AS ProductName,pot.m3_trim_code AS m3TrimCode,pot.colour_id AS trimColorId,tc.colour as trimColor,pot.purchase_order_id as trimPurchaseOrderId,i.request_no AS indentCode,v.vendor_name AS vendorName,pof.po_quantity AS poQuantity,pot.po_quantity AS trQuantity,ifa.quantity AS indentQuantity,itr.quantity AS indentTQuantity,itr.size AS trimsize,ifa.size AS fabricSize
