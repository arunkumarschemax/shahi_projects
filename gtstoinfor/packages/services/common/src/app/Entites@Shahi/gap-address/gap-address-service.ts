...
@Injectable()
export class GapAddressService {
    constructor(
        @InjectDataSource()
        private dataSource: DataSource,
        private repo: GapAddressRepository
    ) { }

    async gapSaveAddressInfo(formData: any): Promise<CommonResponseModel> {
        ...
        try {
            ...
            const deleteData = await transactionManager.getRepository(GapAddressEntity).delete({})
            if (deleteData) {
                flag.add(true)
            } else {
                flag.add(false)
            ...
            for (const data of convertedData) {
                if (data.Destination != null) {
                    ...
                    const addObj = new GapAddressEntity()
                    ...
                    const addSave = await transactionManager.getRepository(GapAddressEntity).save(addObj)
                    if (addSave) {
                        flag.add(true)
                    } else {
                        flag.add(false)
    ...
    async getGapAddressInfoByCountry(req: any): Promise<CommonResponseModel> {
        try {
