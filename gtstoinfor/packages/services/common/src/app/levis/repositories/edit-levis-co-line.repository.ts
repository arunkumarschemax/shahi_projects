...
@Injectable()
export class EditLevisCOLineRepository extends Repository<EditLevisCOLineEntity> {

    constructor(@InjectRepository(EditLevisCOLineEntity) private editlevisCoLineRepository: Repository<EditLevisCOLineEntity>
    ) {
        super(editlevisCoLineRepository.target, editlevisCoLineRepository.manager, editlevisCoLineRepository.queryRunner);
    }

    async getEditCoLineData(req: LevisOrderFilter): Promise<any[]> {
  ...
  // }
    ...
    async getEditCoPoNumber(): Promise<any[]> {
        const query = this.createQueryBuilder('co')
