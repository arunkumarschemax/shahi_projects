    ...
    async getBomInfoAgainstStyle(req:StyleNumberDto):Promise<CommonResponseModel>{
        try{
            ...
            if(data.length >0){
                ...
                const infoModel = new TrimInfoModel(data[0]?.styleId,data[0]?.style,data[0]?.style_name,data[0]?.season,bomInfoModel,data[0].item,data[0].po_number,data[0].msc,data[0].gender)
