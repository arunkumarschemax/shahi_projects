...
export class StyleRepo extends Repository<StyleEntity> {
    ...
    async getBomInfoAgainstStyle(req:StyleNumberDto):Promise<any[]>{
        ...
        .select(`sty.id as styleId,sty.style,sty.style_name,bom.item_name,bom.description,bom.im_code,bom.item_type,bom.use,stco.combination,stco.primary_color,stco.secondary_color,stco.logo_color,bom.id as bomId,stco.id as styleComboId,stco.color,bom.uom,bom.qty,dpom.po_number,dpom.item`)
