...
export class OrdersChildRepository extends Repository<OrdersChildEntity> {
    ...
    async getComparisionphaseData(req: YearReq):Promise<any[]>{
        const query = ` WITH RankedVersions1 AS (
            SELECT
                YEAR,
                VERSION,
                order_plan_number,
               ...
               planned_exf,wh,
                prod_plan_type,
                ROW_NUMBER() OVER (PARTITION BY  prod_plan_type ORDER BY VERSION DESC) AS version_rank            FROM orders_child
        ),
             RankedVersions AS (
            SELECT
                YEAR,
        ...
        // const result = await db.query(query, decodedData);
