...
export class OrdersRepository extends Repository<OrdersEntity> {
    ...
    async getQtyChangeData(): Promise<any[]> {
        const query = this.createQueryBuilder('oc')
            .select(`oc.production_plan_id, oc.item_code, oc.itemName, oc.order_status, oc.fr_fabric_name, oc.contracted_date,oc.requested_wh_date, od.created_at, od.old_val, od.new_val, od.version`)
    ...
    async getWharehouseDateChangeData(): Promise<any> {
        const query = this.createQueryBuilder('oc')
            .select(`oc.production_plan_id, oc.item_code, oc.itemName, oc.order_status, oc.fr_fabric_name, oc.order_qty_pcs, oc.contracted_date, od.created_at, od.old_val, od.new_val, od.version`)
            .leftJoin(OrdersDifferenceEntity, 'od', 'od.prod_plan_id = oc.production_plan_id')
    ...
    async getContractDateChangeData(): Promise<any> {
        const query = this.createQueryBuilder('oc')
            .select(`oc.production_plan_id, oc.item_code, oc.itemName, oc.order_status, oc.fr_fabric_name, oc.order_qty_pcs, oc.requested_wh_date, od.created_at, od.old_val, od.new_val, od.version`)
            .leftJoin(OrdersDifferenceEntity, 'od', 'od.prod_plan_id = oc.production_plan_id')
