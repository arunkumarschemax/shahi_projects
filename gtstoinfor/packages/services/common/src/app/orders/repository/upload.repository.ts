...
export class FileUploadRepository extends Repository<FileUploadEntity> {
    ...
    async getFilesData(): Promise<any[]> {
        const query = this.createQueryBuilder('fup')
            .select(`fup.id as fileId , fup.file_name as fileName , fup.file_path as filePath, fup.created_at as uploadedDate, fup.created_user as createdUser, fup.dpom_status as status, fup.crm_status as crmStatus`)
