...
import { CategoryActivateReq, CategoryReq, ColumnActivateReq, ColumnReq, WeightActivateReq, WeightReq } from "@project-management-system/shared-models"
import CategoryForm from "./weight-form"
...
export const WeightView = () => {
    ...
    const deletecolumn = (rowData) => {
        ...
        const req = new WeightActivateReq(rowData.weightId,'admin',rowData.versionFlag,rowData.isActive)
        service.activeteOrDeactivateWeight(req).then(res => {
            if(res.status){
                getAllData()
    ...
    const updatecolumn = (data:WeightReq) => {
        service.updateWeight(data).then(res => {
            if(res.status){
            ...
            render: (text, rowData) => (
              <span>   
                ...
                <Divider type="vertical" />
                  ...
                  title={
                    rowData.isActive
                      ? 'Are you sure to Deactivate Weight ?'
                      :  'Are you sure to Activate Weight ?'
                  }
                >
                  <Switch  size="default"
    ...
    return (
      ...
      extra={
        <Link to="/trim-master/weight/weight-form">
          <span 
          // style={{ color: "white" }}
          >
    ...
    >
     ...
     <Col span={5}>
           ...
           <Alert type='success' message={'Total weight: ' + data.length} style={{fontSize:'15px'}} />
        </Col>
        <Col span={5}>
