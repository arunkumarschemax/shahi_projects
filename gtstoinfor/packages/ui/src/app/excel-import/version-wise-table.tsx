...
import React, { useEffect, useRef, useState } from 'react';
import { Form } from "react-router-dom";
import { Button, Card, Descriptions, Input, Table, message } from 'antd';
import Highlighter from 'react-highlight-words';
import { Excel } from 'antd-table-saveas-excel';
...
console.log(stateData.VersionData[0].order_plan_number,'000000000000');
  ...
  const  getversions=()=>{
        const req =new ordersPlanNo(stateData.VersionData[0].order_plan_number)
        service.getversion(req).then((res)=>{
            if(res.status){
                setData(res.data);
...
return (
 ...
 <Card title={<span style={{ color: 'black' }}>Order Plan Number :<span style={{color:'black'}}>{data[0]?.order_plan_number}</span></span>} headStyle={{fontWeight:'bold'}} extra={<Link to='/excel-import/grid-view'><span style={{color:'white'}}><Button type="primary">View</Button></span></Link>}> 
{/* <Card title={<span style={{ color: 'black' }}>Order Plan Number: <span style={{ color: 'black' }}>{data[0]?.order_plan_number}</span></span>} headStyle={{ fontWeight: 'bold' }} extra={<Link to='/grid-view'><span style={{ color: 'white' }}><Button type="primary">View</Button></span></Link>}>
   */}
