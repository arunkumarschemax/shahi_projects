...
export interface PlantWisePoOrdersGraphProps { }

export function PlantWisePoOrderGraph() {
  const [plantData, setPlantProData] = useState<any[]>([]);
  const service = new NikeService();
  const form = useForm

  useEffect(() => {
    getPlantWisePoOrders();
  }, [])


  const getPlantWisePoOrders = () => {
    service.getPlantWisePoOrders().then(res => {
      if (res.status) {
        setPlantProData(res.data);
      } else {
        if (res.data) {
            setPlantProData([]);
  ...
  const plantName = plantData.map(i => { return i.plant_name });
  const poCount = plantData.map(i => { return Number(i.poCount) });
  const itemQty = plantData.map(i => { return Number(i.qty) });
  ...
  const config = {
    ...
    title: {
      text: 'Plant Wise Purchase Orders',
      style: {
