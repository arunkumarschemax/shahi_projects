...
import { CentricService, EddieService, HbService, LevisService, SanmarService } from "@project-management-system/shared-services";
...
const LevisEditColineView = () => {
    ...
    const PoNumber = () => {
        service.getEditCoPoNumber().then(res => {
            if (res.status) {
                setBuyer(res.data)
            }
        })
    ...
    const getData = () => {
        ...
        service.getEditCoLineData(req).then(res => {
            if (res.status) {
                setData(res.data)
            }
            else {
    ...
    const handleExport = (e: any) => {
        ...
        excel.saveAs(`Edit-Co-Line-${currentDate}.xlsx`);
    }
    ...
    const handleConfirmDelete = (record) => {
        ...
        const req1 = new ItemNoDto(null,null,record.po_number)
        service.deleteEditCoLine(req).then(res => {
            if (res.status) {
                getData();
                AlertMessages.getSuccessMessage(res.internalMessage)
                service.updateStatusInOrderAcceptance(req1).then((res) => {
                    if (res.status) {
...
export default LevisEditColineView