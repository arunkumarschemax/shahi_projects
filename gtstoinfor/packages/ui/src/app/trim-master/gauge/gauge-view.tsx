...
import { FinishService, GaugeService} from '@project-management-system/shared-services';
import { FinishDTO, GaugeDTO} from '@project-management-system/shared-models';
import GaugeForm from './gauge-form';


export interface GaugeProps {}

export function GaugeGrid(props: GaugeProps) {
  ...
  const [data, setData] = useState<GaugeDTO[]>([]);
  ...
  const saveGauge = (data:GaugeDTO) => {
    data.isActive = data.isActive?false:true;
    Service.activateOrDeactivateGauge(data).then(res => {
      if (res.status) {
    ...
    const updateData = (data: GaugeDTO) => {
      data.updatedUser = JSON.parse(localStorage.getItem('username'))
      Service.updateGauge(data).then(res => {
        if (res.status) {
  ...
  const columnsSkelton: any = [
    ...
    },
    {
      ...
      dataIndex: 'gauge',
      // responsive: ['lg'],
      sorter: (a, b) => a.gauge.localeCompare(b.gauge),
      sortDirections: ['descend', 'ascend'],
      ...getColumnSearchProps('gauge')
    },
    {
      title: 'Status',
      dataIndex: 'isActive',
  ...
  return (
    ...
    extra={<Link to='/trim-master/gauge/gauge-form' >
      ...
      <br></br>
        <Row gutter={24}>
          <Col span={4}></Col>
          <Col span={5}>
            <Alert type='success' message={'Total Gauge: ' + data.length} style={{fontSize:'15px'}} />
          </Col>
          <Col span={5}>
        ...
        <Drawer bodyStyle={{ paddingBottom: 80 }} title='Update' width={window.innerWidth > 768 ? '50%' : '85%'}
            onClose={closeDrawer} visible={drawerVisible} closable={true}>
              <GaugeForm key={Date.now()}
                updateGauge={updateData}
                isUpdate={true}
                data={selectedData}
