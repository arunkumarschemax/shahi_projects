...
import { AirHoleService, FinishService } from '@project-management-system/shared-services';
import { AirHoleDTO, FinishDTO } from '@project-management-system/shared-models';
import AirHoleForm from './air-hole-form';


export interface AirHoleProps {}

export function AirHoleGrid(props: AirHoleProps) {
  ...
  const [data, setData] = useState<AirHoleDTO[]>([]);
  ...
  const getAllAirHole= () => {
    Service.getAllAirHole().then(res => {
      if (res.status) {
        setData(res.data);
      } else {
        if (res.data) {
  ...
  const saveAirHole = (data:AirHoleDTO) => {
    data.isActive = data.isActive?false:true;
    Service.activateOrDeactivateAirHole(data).then(res => {
      if (res.status) {
    ...
    const updateData = (data: AirHoleDTO) => {
      data.updatedUser = JSON.parse(localStorage.getItem('username'))
      Service.updateAirHole(data).then(res => {
        if (res.status) {
  ...
  const columnsSkelton: any = [
    ...
    },
    {
      ...
      dataIndex: 'airHole',
      // responsive: ['lg'],
      sorter: (a, b) => a.airHole.localeCompare(b.airHole),
      sortDirections: ['descend', 'ascend'],
      ...getColumnSearchProps('airHole')
    },
    {
      title: 'Status',
      dataIndex: 'isActive',
  ...
  return (
    ...
    extra={<Link to='/trim-master/air-hole/air-hole-form' >
      ...
      <br></br>
        <Row gutter={24}>
          <Col span={4}></Col>
          <Col span={5}>
            <Alert type='success' message={'Total AirHole: ' + data.length} style={{fontSize:'15px'}} />
          </Col>
          <Col span={5}>
        ...
        <br></br>
          <Table
          size='small'
          rowKey={record => record.airHoleId}
        ...
        <Drawer bodyStyle={{ paddingBottom: 80 }} title='Update' width={window.innerWidth > 768 ? '50%' : '85%'}
            onClose={closeDrawer} visible={drawerVisible} closable={true}>
              <AirHoleForm key={Date.now()}
                updateAirHole={updateData}
                isUpdate={true}
                data={selectedData}
