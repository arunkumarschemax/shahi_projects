...
import { FinishService, TrimUomService } from '@project-management-system/shared-services';
import { FinishDTO, TrimUomDTO } from '@project-management-system/shared-models';
import TrimUomForm from './trim-uom';


export interface TrimUomProps {}

export function TrimUomGrid(props: TrimUomProps) {
  ...
  const [data, setData] = useState<TrimUomDTO[]>([]);
  ...
  const getAllTrimUom= () => {
    Service.getAllTrimUom().then(res => {
      if (res.status) {
        setData(res.data);
      } else {
        if (res.data) {
  ...
  const saveTrimUom = (data:TrimUomDTO) => {
    data.isActive = data.isActive?false:true;
    Service.activateOrDeactivateTrimUom(data).then(res => {
      if (res.status) {
    ...
    const updateData = (data: TrimUomDTO) => {
      data.updatedUser = JSON.parse(localStorage.getItem('username'))
      Service.updateTrimUom(data).then(res => {
        if (res.status) {
      ...
      <br></br>
        ...
        <Drawer bodyStyle={{ paddingBottom: 80 }} title='Update' width={window.innerWidth > 768 ? '50%' : '85%'}
            onClose={closeDrawer} visible={drawerVisible} closable={true}>
              <TrimUomForm key={Date.now()}
                updateTrimUom={updateData}
                isUpdate={true}
                data={selectedData}
