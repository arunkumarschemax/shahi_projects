...
import { FinishDTO, InnerDiaDTO } from '@project-management-system/shared-models';
import InnerDiaForm from './innerdia-form';


export interface InnerDiaFormProps {}

export function InnerDiaGrid(props: InnerDiaFormProps) {
  ...
  const [data, setData] = useState<InnerDiaDTO[]>([]);
  ...
  const getAllInnerDia= () => {
    Service.getAllInnerDia().then(res => {
      if (res.status) {
        setData(res.data);
      } else {
        if (res.data) {
  ...
  const saveInnerDia = (data:InnerDiaDTO) => {
    data.isActive = data.isActive?false:true;
    Service.activateOrDeactivateInnerDia(data).then(res => {
      if (res.status) {
    ...
    const updateData = (data: InnerDiaDTO) => {
      data.updatedUser = JSON.parse(localStorage.getItem('username'))
      Service.updateInnerDia(data).then(res => {
        if (res.status) {
  ...
  const columnsSkelton: any = [
    ...
    },
    {
      ...
      dataIndex: 'innerDia',
      // responsive: ['lg'],
      sorter: (a, b) => a.innerDia.localeCompare(b.innerDia),
      sortDirections: ['descend', 'ascend'],
      ...getColumnSearchProps('innerDia')
    },
    {
      title: 'Status',
      dataIndex: 'isActive',
  ...
  return (
    ...
    extra={<Link to='/trim-master/innerdia/innerdia-form' >
      ...
      <br></br>
        <Row gutter={24}>
          <Col span={4}></Col>
          <Col span={5}>
            <Alert type='success' message={'Total InnerDia: ' + data.length} style={{fontSize:'15px'}} />
          </Col>
          <Col span={5}>
        ...
        <br></br>
          <Table
          size='small'
          rowKey={record => record.innerDiaId}
        ...
        <Drawer bodyStyle={{ paddingBottom: 80 }} title='Update' width={window.innerWidth > 768 ? '50%' : '85%'}
            onClose={closeDrawer} visible={drawerVisible} closable={true}>
              <InnerDiaForm key={Date.now()}
                updateInnerDia={updateData}
                isUpdate={true}
                data={selectedData}
