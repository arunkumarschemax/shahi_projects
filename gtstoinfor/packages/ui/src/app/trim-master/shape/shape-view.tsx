...
import { FinishService, ShapeService,  } from '@project-management-system/shared-services';
import { FinishDTO, ShapeDto,  } from '@project-management-system/shared-models';
import ShapeForm from './shape-form';


export interface ShapeProps {}

export function ShapeGrid(props: ShapeProps) {
  ...
  const [data, setData] = useState<ShapeDto[]>([]);
  ...
  const saveFunction = (data:ShapeDto) => {
    data.isActive = data.isActive?false:true;
    Service.activateOrDeactivateShape(data).then(res => {
      if (res.status) {
    ...
    const updateData = (data: ShapeDto) => {
      data.updatedUser = JSON.parse(localStorage.getItem('username'))
      Service.updateShape(data).then(res => {
        if (res.status) {
  ...
  return (
    ...
    extra={<Link to='/trim-master/shape/shape-form' >
      ...
      <br></br>
        <Row gutter={24}>
          <Col span={4}></Col>
          <Col span={5}>
            <Alert type='success' message={'Total Shape: ' + data.length} style={{fontSize:'15px'}} />
          </Col>
          <Col span={5}>
        ...
        <Drawer bodyStyle={{ paddingBottom: 80 }} title='Update' width={window.innerWidth > 768 ? '50%' : '85%'}
            onClose={closeDrawer} visible={drawerVisible} closable={true}>
              <ShapeForm key={Date.now()}
                updateShape={updateData}
                isUpdate={true}
                data={selectedData}
