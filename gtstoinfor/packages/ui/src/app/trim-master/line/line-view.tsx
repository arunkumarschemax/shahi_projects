...
import { FinishService, LineService } from '@project-management-system/shared-services';
import { FinishDTO, LineDto } from '@project-management-system/shared-models';
import LineForm from './line-form';


export interface LineProps {}

export function LineGrid(props: LineProps) {
  ...
  const [data, setData] = useState<LineDto[]>([]);
  ...
  const saveFunction = (data:LineDto) => {
    data.isActive = data.isActive?false:true;
    Service.activateOrDeactivateLine(data).then(res => {
      if (res.status) {
    ...
    const updateData = (data: LineDto) => {
      data.updatedUser = JSON.parse(localStorage.getItem('username'))
      Service.updateLine(data).then(res => {
        if (res.status) {
  ...
  return (
    ...
    extra={<Link to='/trim-master/line/line-form' >
      ...
      <br></br>
        <Row gutter={24}>
          <Col span={4}></Col>
          <Col span={5}>
            <Alert type='success' message={'Total Line: ' + data.length} style={{fontSize:'15px'}} />
          </Col>
          <Col span={5}>
