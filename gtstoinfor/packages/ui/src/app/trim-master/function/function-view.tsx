...
import { FinishService, FunctionService } from '@project-management-system/shared-services';
import { FinishDTO, FunctionDTO } from '@project-management-system/shared-models';
import FunctionForm from './function-form';


export interface FunctionProps {}

export function FunctionGrid(props: FunctionProps) {
  ...
  const getAllFunction= () => {
    Service.getAllFunction().then(res => {
      if (res.status) {
        setData(res.data);
      } else {
        if (res.data) {
  ...
  const saveFunction = (data:FunctionDTO) => {
    data.isActive = data.isActive?false:true;
    Service.activateOrDeactivateFunction(data).then(res => {
      if (res.status) {
    ...
    const updateData = (data: FunctionDTO) => {
      data.updatedUser = JSON.parse(localStorage.getItem('username'))
      Service.updateFunction(data).then(res => {
        if (res.status) {
  ...
  return (
    ...
    extra={<Link to='/trim-master/function/function-form' >
      ...
      <br></br>
        <Row gutter={24}>
          <Col span={4}></Col>
          <Col span={5}>
            <Alert type='success' message={'Total Function: ' + data.length} style={{fontSize:'15px'}} />
          </Col>
          <Col span={5}>
