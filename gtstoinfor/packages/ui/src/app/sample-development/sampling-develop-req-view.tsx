...
export const SampleDevNewView = () => {
  ...
  const handleDownload = (sampleRequestId: number, sampleRequestNo: string) => {
    const req = new SampleIdRequest(sampleRequestId);
    service.getAllFilesDataByReqId(req).then(res => {
      if (res.status) {
  ...
  const addFileToZip = async (zip, fileName, fileUrl) => {
    try {
  ...
  const createZipFile = async (filesData: any[], fileName: string) => {
    const zip = new JSZip();
  ...
  const HeaderRow = (props: any) => {
    ...
    const { requestId, requestNo, style, buyerName, expectedDate, indentDate, status, lifeCycleStatus, location, brandName, pch, dispatch, index, file } =
      props;
        ...
        <span style={{ marginLeft: 'auto' }}>
          <span style={{ paddingRight: 20 }}  >
            <Tooltip title='Download Tech Pack'>
              {file !== null ? (<DownloadOutlined onClick={() => handleDownload(requestId, (requestNo).replace(/\//g, "_"))} style={{ fontSize: '15px', marginLeft: '-5px', color: 'blue' }} />) : ('')}
          ...
          <Collapse.Panel
            header={
              <HeaderRow
                ...
                requestId={item.sample_request_id}
              />
            }
            key={index}
            extra={
