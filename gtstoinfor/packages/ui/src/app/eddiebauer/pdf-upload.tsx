...
const EddiePdfUpload: React.FC<IPdfUploadProps> = (props) => {
    ...
    const savePdfFields = () => {
        eddieService.saveEddieOrder(poPdfData).then((res) => {
            if (res.status) {
                onReset()
                if (fileList) {
                    ...
                    eddieService.fileUpload(formData).then((res) => {
                        if (res.status) {
