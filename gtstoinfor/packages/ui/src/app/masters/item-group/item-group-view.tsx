...
import { ItemGroupDisplayvalue, ItemGroupDto, ItemGroupEnum, SizeDto } from '@project-management-system/shared-models';
import AlertMessages from '../../common/common-functions/alert-messages';
// import {PaymentMethodService} from '@project-management-system/shared-services'
import { ItemGroupService, SizeService } from '@project-management-system/shared-services';
import {ItemGroupForm} from './item-group-form'
import SizeForm from '../sizes/size.form';

export interface ItemGroupProps{ }

export const ItemGroupGrid =(Props:ItemGroupProps)=>{
    const [searchText, setSearchText] = useState('');
    const [searchedColumn, setSearchedColumn] = useState('');
    const [drawerVisible, setDrawerVisible] = useState(false);
    const [variantData, setVariantData] = useState<ItemGroupDto[]>([]);
...
const closeDrawer = () => {
  ...
  const saveVariant=(variantData:ItemGroupDto)=>{
variantData.itemGroupId=0;
service.createItemGroup(variantData).then(res=>{
    if(res.status){
  ...
  const updateItems=(Item: ItemGroupDto)=>{
    Item.updatedUser=JSON.parse(localStorage.getItem('username'))
  ...
  const deleteGroup = (size:ItemGroupDto) => {
       ...
       <EditOutlined  className={'editSamplTypeIcon'}  type="edit" 
            onClick={() => {
              if (rowData.isActive) {
                ...
                AlertMessages.getErrorMessage('You Cannot Edit Deactivated ItemGroup');
              }
            }}
        ...
        <Divider type="vertical" />
          <Popconfirm onConfirm={e =>{deleteGroup(rowData);}}
          title={
            rowData.isActive
              ? 'Are you sure to Deactivate ItemGroup ?'
              :  'Are you sure to Activate ItemGroup ?'
          }
        >
          <Switch  size="default"
  ...
  <Table
    ...
    <Drawer bodyStyle={{ paddingBottom: 80 }} title='Update' width={window.innerWidth > 768 ? '50%' : '85%'}
        ...
        <Card headStyle={{ textAlign: 'center', fontWeight: 500, fontSize: 16 }} size='small'>
          <ItemGroupForm key={Date.now()}
            updateItem={updateItems}
            isUpdate={true}
            // saveItem={saveVariant}
            groupData={selectedVariant}
