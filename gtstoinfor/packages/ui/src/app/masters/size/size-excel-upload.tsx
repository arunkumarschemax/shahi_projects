...
import { SizeService } from "packages/libs/shared-services/src/common";
...
export const SizeUpload = () => {
      ...
      const handleUpload = async() => {
        try{
            form.validateFields().then(async () => {
                const res = await service.saveSizeInfo(data).then(res => {
                    setLoading(true);
                    if(res.status){
                        AlertMessages.getSuccessMessage(res.internalMessage)
                        navigate('/eddiebauer/masters/Size/Size-view')
                    } else{
    ...
    return(
        <Card title='Size' extra={<Link to='/eddiebauer/masters/size/size-view' >
