...
export interface FabricTypeFormProps{

    fabricTypeData: FabricTypesDto;
    updateItem:(fabricTypeData:FabricTypesDto)=>void
        isUpdate:boolean;
        closeForm:()=>void;
    ...
    export const FabricTypeForm =(props:FabricTypeFormProps)=>{
        ...
        const savePayment = (fabricTypeData:FabricTypesDto ) => {
            // setDisable(true)
            fabricTypeData.fabricTypeId= 0;
            service.createFabricType(fabricTypeData).then((res) => {
              setDisable(false)
                if (res.status) {
          ...
          const saveData =(values:FabricTypesDto)=>{
            setDisable(false)
            if (props.isUpdate) {
          ...
          return(
            <Card title={<span >Fabric Type</span>}  headStyle={{ backgroundColor: '#69c0ff', border: 0 }} 
...
>
    ...
    layout={'vertical'}
    form={form}
    initialValues={props.fabricTypeData}
    name="control-hooks"
    onFinish={saveData}>

<FormItem name="fabricTypeId" style={{display:'none'}}>
    <Input hidden/>
</FormItem>
