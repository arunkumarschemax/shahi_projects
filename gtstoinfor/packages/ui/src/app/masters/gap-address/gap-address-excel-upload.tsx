...
import { GapAddressService } from "@project-management-system/shared-services";


export const GapAddressUpload = () => {
    const [selectedFile, setSelectedFile] = useState(null);
    const [form] = Form.useForm();
    const [data, setData] = useState([])
    const [columns, setColumns] = useState([]);
    const [values, setValues] = useState([])
    const service = new GapAddressService()
      ...
      const handleUpload = async() => {
        try{
            form.validateFields().then(async () => {
                const res = await service.gapSaveAddressInfo(data).then(res => {
                    setLoading(true);
                    if(res.status){
                        AlertMessages.getSuccessMessage(res.internalMessage)
                        navigate("/pvh/masters/gap-address/gap-address-view")
...
export default GapAddressUpload