...
import { ColorService } from "packages/libs/shared-services/src/common";


export const ColorUpload = () => {
      ...
      const handleUpload = async() => {
        try{
            form.validateFields().then(async () => {
                const res = await service.saveColorInfo(data).then(res => {
                    setLoading(true);
                    if(res.status){
                        AlertMessages.getSuccessMessage(res.internalMessage)
                        navigate('/eddiebauer/masters/color/color-view')
                    } else{
    ...
    return(
        <Card title='Color' extra={<Link to='/eddiebauer/masters/color/color-view' >
