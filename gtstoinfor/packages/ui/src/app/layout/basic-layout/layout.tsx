        ...
        if (route && route.subMenuData && route.subMenuData.length) {
            return (
                ...
                return(
                    ...
                    >
                         {route.subMenuName !== 'Trim Orderss'? (<Menu.Item key={route.subMenuId} ><Link  onClick={() => handleClick(route.subMenuId)} to={route.path}><span><span> {route.icon} <span>{route.subMenuName}</span> </span></span></Link> </Menu.Item> ) : (<></>)}
                    </div>
