  ...
  if (extractedData && Array.isArray(extractedData)) {
    ...
    for (const line of extractedData) {
      ...
      if (gstMatch && !gstNumberExtracted) {
        ...
        const taxableAmountData = extractedData.find((item) =>
          item.content.includes("Taxable Amount (INR) :")
        );
        if (taxableAmountData) {
          const match = taxableAmountData.content.match(
            /Taxable Amount \(INR\) : ([\d.]+)/
          );
          if (match) {
...
export const extractDhlCourierfrieght = async (pdf) => {
  ...
  const chargeRegex =
    /TOTAL FOR SHIPMENT\s+(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d)/;
  const unitQuantityRegex =
    /\s(\d+\/+\d+\/+\d+)\s+(\d+)\s+(\d+)\s+(\d+\.\d+)\s+(\w+)\s+(\d+\.\d+)\s+(\d+\.\d)/;

  for (const line of extractedData) {
    if (line.content.match(/996(\d+)/)) {
      hsnId = linesId;

      const chargeMatch = extractedData.find((cha) =>
        cha.content.match(chargeRegex)
      );
      ...
      const unitQuantityMatch = extractedData.find((rec) =>
        rec.content.match(unitQuantityRegex)
      );
      const unitQuantityFind =
        unitQuantityMatch.content.match(unitQuantityRegex);
      const unitQuantity = unitQuantityFind[4]
        ? parseFloat(unitQuantityFind[4])
        : "-";
      const unitPrice = unitQuantityFind[3]
        ? parseFloat(unitQuantityFind[3])
        : "-";
  ...
  if (extractedData && Array.isArray(extractedData)) {
    ...
    for (const line of extractedData) {
      ...
      if (gstMatch && !gstNumberExtracted) {
        ...
        invoiceDate = invoiceDateData
          ? invoiceDateData.content.replace(/DATE\s+/i, "").trim()
          : "";
        ...
        invoiceNumber = invoiceNumberData
          ? invoiceNumberData.content.replace(/ INVOICE\s+NO\s+/i, "").trim()
          : "";
        ...
        invoiceAmount = invoiceAmountData
          ? invoiceAmountData.content.replace(/TOTAL\s+/i, "").trim()
          : "";
  ...
  for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
    ...
    if (line.match(/^.*?(\(+\w+\s+\w+\s+\w+\)+\s+(\d.+\d+))/)) {
      const descriptionData =
        line.match(/^.*?(\(+\w+\s+\w+\s+\w+\)+\s+(\d.+\d+))/) || [];
      const description = descriptionData[0]
        ? descriptionData[0].replace(/\([^)]*\)|\d+.+\d+/g, "")
        : "";
      ...
      const taxType = taxTypeRegex
        ? taxTypeRegex[0]?.replace(/\([^)]*\)/g, "").replace(/,/g, "")
        : "";
    ...
    if (line.match(/996(\d{3})/)) {
      ...
      const taxPercentageMatch =
        extractedData[hsnId + 1].content.match(taxPercentageRegex);
      const taxPercentage = taxPercentageMatch
        ? parseFloat(taxPercentageMatch[2])
        : 0;
      const taxAmount = taxPercentageMatch
        ? parseFloat(taxPercentageMatch[3])
        : 0;
      ...
      const chargeValue = chargeMatch
        ? chargeMatch[3].trim().replace(/,/g, "")
        : "";
...
export const extractApl = async (pdf) => {
  ...
  for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
    ...
    if (
      line.match(
        /([^\d]+)\s+(\d+)\s+(\d+\.\d+)\s+(\w+)\s+(\d+|.+)\s+(\w+)\s+(\d+\.\d+)\s+(\d+\.\d+)/
      )
    ) {
      const matchData =
        line.match(
    ...
    if (
      line.match(/[A-Z0-9]{4}\s+(\d+\s+%)\s+(\w+)\s+(\d+\.\d+)\s+(\d+\.\d+)/)
    ) {
      const taxMatch =
        line.match(
          /[A-Z0-9]{4}\s+(\d+\s+%)\s+(\w+)\s+(\d+\.\d+)\s+(\d+\.\d+)/
...
export const extractExpeditors = async (pdf) => {
    ...
    if (line.content.match(/^\d{6}$/)) {
      ...
      if (currentHSN) {
        if (currentHSN.taxType === "IGST") {
          currentHSN.amount = (
            parseFloat(currentHSN.charge.replace(/,/g, "")) +
            currentHSN.taxAmount
          ).toFixed(2);
        } else if (currentHSN.taxType === "CGST & SGST") {
          currentHSN.amount = (
            parseFloat(currentHSN.charge.replace(/,/g, "")) +
            currentHSN.taxAmount * 2
          ).toFixed(2);
        }
        structuredHSNLines.push(currentHSN);
      }
      ...
      if (taxPercentage === 18) {
        igst =
          parseFloat(extractedData[hsnId + 6].content.replace(/,/g, "")) || 0;
        taxType = "IGST";
      } else if (taxPercentage === 9) {
        cgst =
          parseFloat(extractedData[hsnId + 6].content.replace(/,/g, "")) || 0;
  ...
  if (currentHSN) {
    if (currentHSN.taxType === "IGST") {
      currentHSN.amount = (
        parseFloat(currentHSN.charge) + currentHSN.taxAmount
      ).toFixed(2);
    } else if (currentHSN.taxType === "CGST & SGST") {
      currentHSN.amount = (
        parseFloat(currentHSN.charge) +
        currentHSN.taxAmount * 2
      ).toFixed(2);
    }
    structuredHSNLines.push(currentHSN);
...
export const extractEfl = async (pdf: PDFDocumentProxy) => {
  ...
  for (const line of allLines) {
    ...
    for (const hsnLine of structuredHSNLines) {
      ...
      if (
        !isNaN(hsnLine.charge) &&
        !isNaN(hsnLine.unitQuantity) &&
        hsnLine.unitQuantity !== 0
      ) {
  ...
  let gstNumberExtracted = false;
  const invoiceDateId =
    /\b\d{1,2}-(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-\d{2}\b/g;
  ...
  if (extractedData && Array.isArray(extractedData)) {
    for (const line of extractedData) {
      ...
      if (gstMatch && !gstNumberExtracted) {
        ...
        const invoiceDateData = extractedData.find((item) =>
          item.content.match(invoiceDateId)
        );
        const invoiceDate = invoiceDateData
          ? invoiceDateData.content.match(invoiceDateId)
          : "";

        const invoiceNumberData = extractedData.find((item) =>
          item.content.match(invoiceNumberId)
        );
        const invoiceNumberNew = invoiceNumberData
          ? invoiceNumberData.content.split("|")[1]
          : "";
...
export const extractOocl = async (pdf) => {
  ...
  for (const line of extractedData) {
    if (line.content.includes("996")) {
      ...
      const taxAmountContent = extractedData[hsnId + 14].content.replace(
        /,/g,
        ""
      );
      ...
      const charge = parseFloat(
        extractedData[hsnId + 2].content.replace(/,/g, "")
      );
      const unitPrice = (charge / unitQuantity).toFixed(2);
  ...
  if (extractedData && Array.isArray(extractedData)) {
    for (const line of extractedData) {
      ...
      if (gstMatch && !gstNumberExtracted) {
        ...
        const vendorNameData = extractedData.find(
          (item) => item.id === vendorNameId
        );
        const venName = vendorNameData ? vendorNameData.content : "";

        const invoiceDateData = extractedData.find(
          (item) => item.id === invoiceDateId
        );
        const invoiceDate = invoiceDateData
          ? invoiceDateData.content.replace(/^INVOICE DATE:\s*/, "")
          : "";

        const invoiceNumberData = extractedData.find(
          (item) => item.id === invoiceNumberId
        );
        const invoiceNumber = invoiceNumberData
          ? invoiceNumberData.content.replace(/^INVOICE NUMBER:\s*/, "")
          : "";
...
export const extractMaersk = async (pdf) => {
  ...
  for (const line of extractedData) {
    if (line.content.includes("996")) {
      ...
      const taxPercentage = taxPercentageMatch
        ? parseFloat(taxPercentageMatch[0])
        : 0;
      ...
      const taxAmountContent = extractedData[hsnId + 17].content.replace(
        /,/g,
        ""
      );
      const taxAmount = parseFloat(taxAmountContent).toFixed(2);
      ...
      const charge = parseFloat(
        extractedData[hsnId + 14].content.replace(/,/g, "")
      );
      ...
      currentHSN = {
        description:
          extractedData[hsnId - 6].content + extractedData[hsnId - 5].content,
        HSN: line.content.includes("996")
          ? line.content.match(/\d+/)
          : line.content.trim(),
        unitQuantity: unitQuantity,
...
export const extractMsn = async (pdf) => {
  ...
  if (hsns.length === hsnTaxes.length) {
    ...
    for (const rec of hsns) {
      ...
      const hsnTaxesArray = hsnTaxes[hsnIndex]
        .split(" ")
        .filter((rec) => rec != "");
      console.log(hsnTaxesArray);
      const taxType =
        hsnTaxesArray[1] == 0 && hsnTaxesArray[3] == 0 && hsnTaxesArray[5] == 0
          ? "No Tax"
          : hsnTaxesArray[1] == 0 && hsnTaxesArray[3] == 0
            ? "IGST"
            : "CGST & SGST";
      const hsnLinesData = {
        ...
        taxAmount:
          taxType === "No Tax"
            ...
            : taxType === "IGST"
              ? hsnTaxesArray[6]
              : Number(hsnTaxesArray[2]) + Number(hsnTaxesArray[4]),
        taxPercentage:
          taxType === "No Tax"
            ? 0
            : taxType === "IGST"
  ...
  if (extractedData && Array.isArray(extractedData)) {
    ...
    for (const line of extractedData) {
      ...
      if (gstMatch && !gstNumberExtracted) {
        ...
        invoiceAmount = invoiceAmountData
          ? invoiceAmountData.content.replace(/E & O E\s+Total\s+/i, "").trim()
          : "";
...
export const extractFredexfrieght = async (pdf) => {
  ...
  if (hsns.length != 0 && hsnTaxes.length != 0) {
    ...
    for (const rec of hsnTaxes) {
      ...
      const hsnTaxesArray = hsnTaxes[hsnIndex]
        ...
        .filter((rec) => rec != "");
      console.log(hsnTaxesArray);
      const unitPrice = hsnTaxesArray.filter((rec) =>
        rec.match(/([\d,]*\d+\.\d{2})/)
      );
      console.log(unitPrice);
      const taxType =
        hsnTaxesArray.length < 12
          ? "No Tax"
          : hsnTaxesArray[1] == 0 && hsnTaxesArray[3] == 0
            ? "IGST"
            : "CGST & SGST";
      const qtySplittedArray = qtyArray[0]
        .split(" ")
        .filter((rec) => rec != "");
      const unitQuantity = qtySplittedArray.find((rec) =>
        rec.match(/([\d,]*\d+\.\d{2})/)
      );
      const hsnLinesData = {
        ...
        taxAmount:
          taxType === "No Tax"
            ...
            : taxType === "IGST"
              ? hsnTaxesArray[6]
              : Number(unitPrice[5]) + Number(unitPrice[6]),
        taxPercentage:
          taxType === "No Tax"
            ? 0
            : taxType === "IGST"
        ...
        quotation: null,
        unitPrice: unitPrice.length
          ? taxType === "No Tax"
            ? unitPrice[2]
            : unitPrice[2]
          : 0,
        innAmount: unitPrice.length
          ? taxType === "No Tax"
            ? unitPrice[2]
            : unitPrice[2]
          : 0,
        igst: unitPrice.length
          ? taxType === "IGST"
            ? unitPrice[7]
            : unitPrice[7]
          : 0,
        cgst: unitPrice.length
          ? taxType === "CGST & SGST"
            ? unitPrice[5]
            : unitPrice[5]
          : 0,
        sgst: unitPrice.length
          ? taxType === "CGST & SGST"
            ? unitPrice[6]
            : unitPrice[6]
          : 0,
        charge:
          (unitPrice[4] || "").trim() !== "" ? unitPrice[4] : unitPrice[3],
        unitQuantity: unitQuantity ? unitQuantity : 0,
        amount:
          taxType === "No Tax" ? hsnTaxesArray[hsnTaxesArray.length - 1] : 0,
        description: hsnTaxesArray
          .splice(1, hsnTaxesArray.length)
          .filter(
            (rec) =>
              !rec.match(/([\d,]*\d+\.\d{2})/) &&
              !currency_list.includes(rec) &&
              !itemCodesArray.includes(rec)
          )
          .join(" "),
      };
      structuredHSNLines.push(hsnLinesData);
  ...
  if (extractedData && Array.isArray(extractedData)) {
    ...
    for (let i = 0; i < extractedData.length; i++) {
      ...
      if (
        line.content.match(
          /GSTIN:\s[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/
        )
      ) {
        continue;
      }
      ...
      if (gstMatch && !gstNumberExtracted) {
        ...
        invoiceDate = invoiceDateData
          ? invoiceDateData.content.match(invoiceDateRegex)[1]
          : "";
        ...
        invoiceNumber = invoiceNumberData
          ? invoiceNumberData.content.match(invoiceNumberRegex)[1]
          : "";
        ...
        const invoiceAmount = structuredHSNLines
          .reduce((add, hsnLine) => {
            ...
            const sgst = parseFloat(hsnLine.sgst) || 0;
            const innAmount =
              hsnLine.innAmount && typeof hsnLine.innAmount === "string"
                ? parseFloat(hsnLine.innAmount.replace(/,/g, ""))
                : 0;
        ...
        for (const hsnLine of structuredHSNLines) {
          if (hsnLine.taxType === "IGST") {
            igst = (parseFloat(igst) + parseFloat(hsnLine.isgt || 0)).toFixed(
              2
            );
          } else if (hsnLine.taxType === "CGST & SGST") {
            cgst = (parseFloat(cgst) + parseFloat(hsnLine.cgst || 0)).toFixed(
              2
            );
            sgst = (parseFloat(sgst) + parseFloat(hsnLine.sgst || 0)).toFixed(
              2
            );
          }
        }
...
export const extractFredexCourier = async (pdf) => {
  ...
  let cgstId = null;
  let linesId = 0;
  const mainMatch = extractedData.find((line) =>
    line.content.match(/Charges\s+KA\s+IGST\s+18%\s+Total/)
  );
  if (mainMatch) {
    taxId = mainMatch.id;
  }
  for (const line of extractedData) {
    ...
    if (hsnMatch) {
      ...
      const cgstMatch = extractedData.find((line) =>
        line.content.match(/KA (SGST|CGST) (\d+%)/)
      );
      cgstId = cgstMatch.id;
      if (cgstMatch) {
        ...
        const igstMatch = extractedData
          .find((rec) => rec.id === taxIdArray.join("-"))
          .content.match(/(\d{1,3}(,\d{3})*\.\d{2})/g);
        const igst = igstMatch
          ? parseFloat(igstMatch[1].replace(/,/g, "")).toFixed(2)
          : "0.00";

        const cgstMatch = extractedData
          .find((rec) => rec.id === cgstIdArray.join("-"))
          .content.match(/\d{1,3}(?:,\d{3})*(?:\.\d+)?/);
        const cgst = cgstMatch
          ? parseFloat(cgstMatch[0].replace(/,/g, "")).toFixed(2)
          : "0.00";

        const sgstMatch = extractedData
          .find((rec) => rec.id === cgstIdArray.join("-"))
          .content.match(/\d{1,3}(?:,\d{3})*(?:\.\d+)?/);
        const sgst = sgstMatch
          ? parseFloat(sgstMatch[0].replace(/,/g, "")).toFixed(2)
          : "0.00";
  ...
  if (extractedData && Array.isArray(extractedData)) {
    ...
    for (const line of extractedData) {
      ...
      if (gstMatch && !gstNumberExtracted) {
        ...
        invoiceAmount = invoiceAmountData
          ? invoiceAmountData.content
            .replace(/ Total\s+Amount\s+Due\s+INR\s+/i, "")
            .trim()
          : "";
...
export const extractPDFDataToLinesData = async (
  ...
  for (let pageNumber = 1; pageNumber <= numPages; pageNumber++) {
    ...
    for (let i = 0; i < textContent.items.length; i++) {
      ...
      if (i > 0) {
        ...
        if (
          Math.abs(item.transform[5] - prevItem.transform[5]) <=
          verticalTolerance
        ) {
  ...
  for (const line of allLines) {
    ...
    if (currentHSN && currentHSN.HSN && !currentHSN.taxType) {
      if (!line.content.includes("996")) {
        ...
        let lineContent = line.content.trim(); // Trim the value

        currentHSN.description += " " + lineContent;
      }
    }
...
export const extracteWiderlogistics = async (pdf) => {
  ...
  for (const line of extractedData) {
    if (line.content.match(/^\d{6}$/)) {
      ...
      const description = extractedData[hsnId - 4].content;
