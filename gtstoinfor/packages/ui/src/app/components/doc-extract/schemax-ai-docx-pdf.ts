...
// /\d+\.\d+\s+\d+(,|.)\d+(,|.)\d+(\s|\d)+(,|.)\d+(\d|\.)+\s+\d+(\d|\.)+/ 1 scanwell
// /\d+\.\d+\s+\d+(,|.)\d+(,|.)\d+\s+\d+\s+\d+(,|.)\d+(\.|\d)+\s+/ 2 scanwell
//CGST\s+(\d+\.\d+)\s*%\s*(\d+\.\d+)//
//SGST\s+(\d+\.\d+)\s*%\s*(\d+\.\d+)//
...
export const extractedRahat = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (matchData.length > 0) {
            // const description = matchData[0].match(/\s+[A-Z].+\s+-/g, "").replace(/(\-)/g,"");
            const descriptionMatch = line.match(/\s+[A-Z].+\s+-/g);
            const description = descriptionMatch ? descriptionMatch[0].replace(/(\-)/g, "") : null;
    ...
    const invoiceNumberRegex = /160-[A-Z0-9]{12}/;
    const invoiceAmountRegex = /\s+Total in INR\s:/;
    const igstRegex = /\d+\s+\d+(,|\.\d)\d\s+\d+(,|\.\d)\d/;
    const cgstRegex = /\d+\s+\d+(,|\.\d)\d\s+\d+(,|\.\d)\d/;
    const sgstRegex = /\d+\s+\d+(,|\.\d)\d\s+\d+(,|\.\d)\d/;
        ...
        if (extractedData && Array.isArray(extractedData)) {
            for (const line of extractedData) {
                ...
                if (gstMatch && !gstNumberExtracted) {
                    ...
                    // }, 0).toFixed(2);
                    // console.log(invoiceAmount, "innnv")

                    const igstData = extractedData.find((item) => item.content.match(igstRegex));
                    igst = igstData ? igstData.content.match(/\d+\s+\d+(,|\.\d)\d\s+\d+(,|\.\d)\d/)[0].replace(/\d+\s+\d+(,|\.\d)\d\s/g, "").trim() : "";

                    // const cgstData = extractedData.find((item) => item.content.match(cgstRegex));
                    // cgst = cgstData ? cgstData.content.match(/\d+\s+\d+(,|\.\d)\d\s+\d+(,|\.\d)\d/)[0].replace(/\d+\s+\d+(,|\.\d)\d\s/g,""): 0.00;

                    // const sgstData = extractedData.find((item) => item.content.match(sgstRegex));
                    // sgst = sgstData ? sgstData.content.match(/\d+\s+\d+(,|\.\d)\d\s+\d+(,|\.\d)\d/)[0].replace(/\d+\s+\d+(,|\.\d)\d\s/g,""): 0.00;
                    // let igst = "0.00";
                    // let cgst = "0.00";
                    // let sgst = "0.00";
...
export const extractedSolitaire = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    const invoiceNumberRegex = /Job\s+No:|MAA-[A-Z0-9]{2}-[A-Z0-9]{5}-[A-Z0-9]{5}-[A-Z0-9]{5}/;
    const invoiceCurrency = "INR";
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        ...
        if (extractedData && Array.isArray(extractedData)) {
            for (const line of extractedData) {
                const gstMatch = line.content.match(/[0-9]{2}[A-Z]{5}[0-9]{4}\s[A-Z0-9]{4}|GST\s+NO.\s+[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z0-9]{4}/g);
                if (gstMatch && !gstNumberExtracted) {
                    const gstNumber = gstMatch[0].replace(/\s+/g, "") && gstMatch[0].replace(/GST\s+NO\.\s+/g, "");

                    venName = 'SOLITAIRE LOGISTIC   PVT. LTD';

                    const invoiceDateData = extractedData.find((item) => item.content.match(invoiceDateRegex));
                    invoiceDate = invoiceDateData ? invoiceDateData.content.match(invoiceDateRegex) : "";

                    const invoiceNumberData = extractedData.find((item) => item.content.match(invoiceNumberRegex));
                    invoiceNumber = invoiceNumberData ? invoiceNumberData.content.replace(/Job\s+No:/g, "") : "";
...
export const extractcogoPort = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    console.log(allLines)
    const extractedData = allLines;
    // const amountRegex = new RegExp(matchData[8]); 
    // const amountData = extractedData.find((item) => item.content.match(amountRegex));
    // const amount = amountData ? amountData.content.match(amountRegex)[0] : '';
    // /([^\"])+(\d+)\s+\w+\s+([\d.]+)\s+([\d.]+)\s+(\w)\s+([\d.]+%)\s+([\d.]+)\s+\d+\s+\d+\s+([\d.]+)\s+([\d.]+)/
    const structuredHSNLines = [];
    let currentHSN = null;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (extractedData[hsnId] && extractedData[hsnId].content) {
            const matchData = line.match(/(\d+)\s+\w+\s+([\d.]+)\s+([\d.]+)\s+(\w)\s+([\d.]+%)\s+([\d.]+)\s+\d+\s+\d+\s+([\d.]+)\s+([\d.]+)/);

            if (matchData) {
                const HSN = matchData[1];
                const amount = matchData[8];
                ...
                const unitPrice = parseFloat(matchData[3]).toFixed(2);
                const charge = matchData[6];
                const taxPercentage = matchData[5].replace(/%/g, "");
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            const gstMatch = line.content.match(/[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/g);
            if (gstMatch && !gstNumberExtracted) {
                venName = 'COGOPORT PRIVATE LIMITED';
                const gstNumber = gstMatch[0];
                let invoiceNumber = '';
                let invoiceDate = '';
    ...
    console.log("SCANWELL PDF DATA", JSON.stringify(allLines, null, 2))
    console.log("Combined Data", structuredHSNLines);
    console.log("PDF JSON DATA", JSON.stringify(structuredHSNLines, null, 2));
    return {
        extractedData: InvoiceLines[0],
...
export const extractmatrix = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        const matchData = line.match(/(996\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)/);

        if (matchData) {
            const HSN = matchData[1];
            const charge = parseFloat(matchData[2]);
            const taxAmount = parseFloat(matchData[3]);
            const amount = parseFloat(matchData[4]);
    ...
    if (extractedData && Array.isArray(extractedData)) {
        ...
        for (const line of extractedData) {
            const gstMatch = line.content.match(/GST No\s+:\s+[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/);
            if (gstMatch && !gstNumberExtracted) {
                const gstNumber = gstMatch[0];

                venName = 'MATRIX FREIGHT SYSTEMS PRIVATE LIMITED';
...
export const extractScanWell = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    console.log(allLines)
    const extractedData = allLines;
    // /(\w.+)\s+(\d+\.\d+)\s+([\d,.]+)\s+(\d+)\s+([\d,.]+)\s+(\d+\.\d+)/ description regex 
    // const description = matchData[1].trim();

    // const currencyRegex=/(INR|USD)/;


    // const unitQ = extractedData.find((item) => item.content.match(unitQuantity));
    // const currencyRegex = /(INR|USD)\s/
    // const currencyData = extractedData.find((item) => item.content.match(currencyRegex));
    // currency = currencyData ? currencyData.content : '';
    const structuredHSNLines = [];
    let currentHSN = null;
    let currentTax = null;
    const taxRegMatch = /(\d[,.0-9]*)\s*(?:IGST|CGST|SGST|GST)+\s*(\d+(?:\.\d+)?%)/;
    ...
    const hsnRegexMatch = /(SAS|SAC|HSN)\s+\w+:\s+(\d+)/;
    const hsnMatchElement = extractedData.find((item) => item.content.match(hsnRegexMatch));
    const hsnMatch = hsnMatchElement?.content?.match(hsnRegexMatch);
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (matchData) {
            ...
            let description = "";
            if ((line.includes("INR") || line.includes("USD")) && unitQuantity) {
                const indexOfCurrency = line.indexOf("INR") !== -1 ? line.indexOf("INR") : line.indexOf("USD");
                const indexOfQuantity = line.indexOf(unitQuantity);
                if (indexOfCurrency !== -1 && indexOfQuantity !== -1) {
                    description = line.substring(indexOfCurrency + 3, indexOfQuantity).trim();
                }
            } else {
                let descriptionStartIndex;
                // eslint-disable-next-line no-loop-func
                extractedData.find((item, index) => {
                    if (((index > Number(previousHsnIndex)) && (index <= Number(currentHsnIndex))) && item.content.match(/(INR|USD)\s+(\w)/)) {
                ...
                const descriptionArray = extractedData.filter((item, index) => (index >= Number(descriptionStartIndex) && index < currentHsnIndex));
                description = descriptionArray.map(rec => rec.content).join('').replace(/(INR|USD)/g, "")
            }
    ...
    const invoiceNumberRegex = /BLR[A-Z0-9]{11}/;
    const invoiceCurrency = 'INR';
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            const gstMatch = line.content.match(/[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/g);
            if (gstMatch && !gstNumberExtracted) {
                venName = 'Scanwell Logistics India (Pvt) Ltd';
                const gstNumber = gstMatch[0];
                let invoiceNumber = '';
                let invoiceDate = '';

                const invoiceDateData = extractedData.filter((item) => item.content.match(invoiceDateRegex));
                invoiceDate = invoiceDateData ? invoiceDateData[1].content : '';
...
export const extractnewBlobe = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        // /\s*((\d+|.)\d{1,3}(,\d{3})*\.\d+\s+)+\d{1,3}(,\d{3})*\.\d+\s*/
        if (matchData) {
            const currentHsnIndex = Number(dataIndex);
            let taxMatchIndex;
            // eslint-disable-next-line no-loop-func
            ...
            const taxMatch = taxMatchData?.content?.match(/\s*((\d+|.)\d{1,3}(,\d{3})*\.\d+\s+)+\d*(,\d{3})*\.\d+\s*/);
            ...
            if (taxMatch) {
                ...
                const charge = parseFloat(taxValues[1].replace(/,/g, "")).toFixed(2);

                console.log(taxMatch, "tty")
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            ...
            if (gstMatch && !gstNumberExtracted) {
                ...
                const invoiceAmount = Math.round(structuredHSNLines.reduce((add, hsnLine) => {
                    const charge = parseFloat(hsnLine.charge) || 0;
                    const taxAmount = parseFloat(hsnLine.taxAmount) || 0;
                    return add + charge + taxAmount;
                }, 0)).toFixed(2);
...
export const extractLxpantos = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (matchData.length > 0) {
            ...
            // /.*\d+\s+\d+\s+/ trim after numbers
            // /^.*?\d+.*?\d+/ trim before numbers
            if (hsnIndex !== -1) {
                const description = mainDatas.slice(0, hsnIndex).join(' ').replace(/^.*?\d+.*?\d+/g, "");
...
export const extractToll = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (matchData.length > 0) {
            ...
            const description = matchResult ? matchResult[1].replace(/IGST|SGST|CGST/g, '').replace(/\d/g, "") : null;
    ...
    for (const line of allLines) {
        ...
        if (currentHSN && currentHSN.HSN && !currentHSN.taxType) {
            if (!line.content.includes("996")) {
                ...
                if (currentHSN && line && line.content) {
                    if (currentHSN.descriptionDataMatch !== null && currentHSN.descriptionDataMatch !== undefined) {
                        currentHSN.descriptionDataMatch += ' ' + line.content.trim();
                        currentHSN.description = currentHSN.descriptionDataMatch.replace(/\d.+/g, "").replace(/-/g,"").replace(/\d/g,"");
                    } else {
                        currentHSN.descriptionDataMatch = line.content.trim();
        ...
        if (line.content.includes("=") && currentHSN) {
            ...
            if (taxAmountMatch) {
                ...
                let igst = 0.00,
                    ...
                    sgst = 0.00;
                if (currentHSN.taxPercentage === 18) {
                    igst = currentHSN.taxAmount;
                    cgst = 0.00;
                    sgst = 0.00;
                } else if (currentHSN.taxPercentage === 9) {
                    igst = 0.00;
                    cgst = currentHSN.taxAmount;
                    sgst = currentHSN.taxAmount;
                }
                currentHSN.igst = igst.toFixed(2);
                currentHSN.cgst = cgst.toFixed(2);
                currentHSN.sgst = sgst.toFixed(2);
            }
        }
        if (currentHSN) {
...
export const extractNagel = async (pdf) => {
    ...
    for (const line of extractedData) {
        if (line.content.match(/^\d{8}$/)) {
            ...
            const charge = parseFloat(extractedData[hsnId - 16].content.replace(/,/g, ""));
            const amount = parseFloat(extractedData[hsnId - 8].content.replace(/,/g, ""));
    ...
    if (extractedData && Array.isArray(extractedData)) {
        ...
        for (const line of extractedData) {
            ...
            if (gstMatch && !gstNumberExtracted) {
                const gstNumber = gstMatch[0];
                venName = 'KUEHNE NAGEL PRIVATE LIMITED';
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            ...
            if (gstMatch && !gstNumberExtracted) {
                ...
                const gstNumber = gstMatch[0].replace(/GSTIN#\s/g, "");
                let invoiceNumber = '';
                let invoiceDate = '';
                let invoiceAmount = '';
                let igst = '';
                ...
                const invoiceAmountData = extractedData.find((item) => item.content.match(invoiceAmountRegex));
                invoiceAmount = invoiceAmountData ? invoiceAmountData.content.replace(/ TOTAL INR /g, "").trim() : '';

                const invoiceDateData = extractedData.find((item) => item.content.match(invoiceDateRegex));
                invoiceDate = invoiceDateData ? invoiceDateData.content.replace(/ INVOICE DATE /g, "").trim() : '';

                const invoiceNumberData = extractedData.find((item) => item.content.match(invoiceNumberRegex));
                invoiceNumber = invoiceNumberData ? invoiceNumberData.content.replace(/TAX\s+INVOICE\s+|Original for Recipient/g, "").trim() : '';

                const igstData = extractedData.find((item) => item.content.match(igstRegex));
                igst = igstData ? igstData.content.replace(/ADD IGST /g, "").trim() : '' || 0.00;

                const cgstData = extractedData.find((item) => item.content.match(cgstRegex));
                cgst = cgstData ? cgstData.content.replace(/ADD CGST /g, "").trim() : '' || 0.00;

                const sgstData = extractedData.find((item) => item.content.match(sgstRegex));
                sgst = sgstData ? sgstData.content.replace(/ADD SGST /g, "").trim() : '' || 0.00;
...
export const extractTotalTransport = async (pdf) => {
    ...
    for (const line of extractedData) {
        if (line.content.match(/996\d{3}$/)) {
            ...
            const description = extractedData[hsnId + 1].content;
            if (description.trim() !== '') {
                currentHSN = {
                    description: description,
                    HSN: line.content.includes("HSN") ? line.content.match(/\d+/) : line.content.trim(),
...
export const extractTextiles = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (matchData.length > 0) {
            ...
            const hsnMatch = description.match(/\b(998|996)\d+\b/);
            const HSN = hsnMatch ? hsnMatch[0] : '';

            if (HSN) {
                description = description.replace(HSN, '').replace(/\d+/g, "").trim();
            }
    ...
    let gstNumberExtracted = false;
    const invoiceDateRegex = /[0-9]{2}-[0-9]{2}-[0-9]{4}/;
    const invoiceNumberRegex = /TCB+[A-Z0-9]+|FIO+[A-Z0-9]+/;
...
//     let linesId = 0;
// // Unicorn Regex
// // /.+\s+\d+(,|.)(\.\d|\d)+\s+\d+\s+\%+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/
...
export const extractedUnicorn = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        const matchData = line.match(/.+\s+\d+(,|.)(\.\d|\d)+\s+\d+\s+\%+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/) || [];

        if (matchData.length > 0) {
            const descriptionMatch = line.match(/.+\s+\d+(,|.)(\.\d|\d)+\s+\d+\s+\%+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g);
            const description = descriptionMatch ? descriptionMatch[0].replace(/\s+\d+(,|.)(\.\d|\d)+\s+\d+\s+\%+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "") : null;

            const HSN = line.match(/(996|998|99)\d+/g, "");

            const unitQuantityMatch = line.match(/\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g,"");
            const unitQuantity =unitQuantityMatch && unitQuantityMatch[0] ? unitQuantityMatch[0].replace(/\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "") : null;

            const unitPriceMatch = line.match(/\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g,"");
            const unitPrice =unitPriceMatch && unitPriceMatch[0] ? unitPriceMatch[0].replace(/\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g,"").replace(/(INR|USD)/g,"").replace(/,/g,"") : null;

            const chargeMatch = line.match(/\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "");
            const charge = chargeMatch && chargeMatch[0] ? chargeMatch[0].replace(/\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "").replace(/,/g, "") : null;

            const taxPercentageMatch = line.match(/\s+\d+\s+\%+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "");
            const taxPercentage = taxPercentageMatch && taxPercentageMatch[0] ? taxPercentageMatch[0].replace(/\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "").replace(/%/g, "") : null;

            const taxAmountMatch = line.match(/\s+\d+(,|.)(\.\d|\d)+\s+\d+\s+\%+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "");
            const taxAmount = taxAmountMatch && taxAmountMatch[0] ? taxAmountMatch[0].replace(/\s+\d+\s+\%+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)+\s+\d+(,|.)(\d|\.\d)+\s+\d+(,|.)(\d|\.\d)+\s+(\w)+\s+(996|998)\d+/g, "").replace(/,/g, "") : "";
        ...
        if (extractedData && Array.isArray(extractedData)) {
            for (const line of extractedData) {
                ...
                if (gstMatch && !gstNumberExtracted) {
                    ...
                    const invoiceDateData = extractedData.find((item) => item.content.match(invoiceDateRegex));
                    invoiceDate = invoiceDateData ? invoiceDateData.content.replace(/(Bill|[A-Z])+\s+(Date|[A-Z])\s+\:\s+/g,""): "";

                    const invoiceNumberData = extractedData.find((item) => item.content.match(invoiceNumberRegex) );
                    invoiceNumber = invoiceNumberData ? invoiceNumberData.content.replace(/(Bill|[A-Z])+\s+(Number|[A-Z])\s+\:\s+/g,"") : "";

                    // const invoiceAmountData = extractedData.find((item) => item.content.match(invoiceAmountRegex) );
                    // invoiceAmount = invoiceAmountData ? invoiceAmountData.content : "";
    ...
    for (const line of allLines) {
        ...
        if (currentHSN && currentHSN.HSN && !currentHSN.taxType) {
            if (!line.content.includes("996")) {
                ...
                currentHSN.description += " " + line.content.trim();
                currentHSN.description = currentHSN.description.replace( /[^a-zA-Z ]/g,"").replace(/(INR|USD)/g,"").replace(/(KG|KGS|CBM)/g,"");
            }
        }
...
//       const matchData = line.match(/(996|998|99)\d+\s+.+/);
...
//         const charge = matchData[5] ? matchData[5].trim() : '';  
...
export const extractedVelogicIndia = async (pdf) => {
    ...
    let currentHSN = null;
    // const uniqueDescriptions = new Set();
    const hsnIndexesArray = [];
    let hsnsBrakIndex = null;
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        const line = extractedData[hsnId].content;
        if (line.includes('TAX INVOICE   COPY'))
            break;
        if (line.includes('Amount in Words:')) {
            hsnsBrakIndex = hsnId - 2;
            break;
        }
        const matchData = line.match(/(996|998|99)\d+\s+.+/) || [];
        if (matchData.length > 0) {
            hsnIndexesArray.push(hsnId);
        }
    }
    const hsnData = [];
    ...
    hsnIndexesArray.forEach((rec, index) => {
        if (hsnIndexesArray[index + 1] - rec == 1) {
            ...
            hsnData.push(`${extractedData[rec].content}${extractedData[hsnIndexesArray[index] + 1].content}`);
        }
    })
    hsnData.forEach(rec => {
        const descriptionMatch = rec.match(/\b(996|998)\d+\s+[A-Z]+(?:\s+[A-Z]+)*\b/g, "");
        const description = descriptionMatch ? descriptionMatch[0].replace(/(996|998|99)\d+\s+/g, "") : null;
        const HSN = rec.match(/(996|998)+\d+/g, "");

        const unitPriceMatch = rec.match(/:+\s+(\d|\d+\.\d)+\s+\*+\s+(\d|\d+\.\d)+\s+/g, "");
        const unitPrice = unitPriceMatch ? unitPriceMatch[0].replace(/\*+\s+(\d|\d+\.\d)+\s+/g, "").replace(/:/g, "") : "";

        const unitQuantityMatch = rec.match(/:+\s+(\d|\d+\.\d)+\s+\*+\s+(\d|\d+\.\d)+\s+/g, "");
        const unitQuantity = unitQuantityMatch ? unitQuantityMatch[0].replace(/:+\s+(\d|\d+\.\d)+\s+\*/g, "").replace(/:/g, "").replace(/\*/g, "") : null;

        const toMatchCharge = rec.match(/(\d+\.\d)+\s+(\d+\.\d)+\s+(\d+\.\d)+\s+(\d+(.|,)(\.\d|\d)+)+\s+((\d+(.|,)(\.\d|\d)+)+|(\d+(.|,)(\.\d|\d)+))+((\d+(.|,)(\.\d|\d)+))+.+/);
        const taxAmount=toMatchCharge[4]?toMatchCharge[4].replace(/,/g,""):"";
        console.log(toMatchCharge, "**toMatchCharge") 

         // const chargeMatch = rec.match(/(\d+(?:,\d{3})*(?:\.\d+)?)/g,"");
        // const charge = chargeMatch ? chargeMatch[chargeMatch.length - 2].replace(/,/g,"") : "";

        const amountMatch=rec.match(/(\d+\.\d)+\s+(\d+\.\d)+\s+(\d+\.\d)+\s+(\d+(.|,)(\.\d|\d)+)+\s+((\d+(.|,)(\.\d|\d)+)+|(\d+(.|,)(\.\d|\d)+))+((\d+(.|,)(\.\d|\d)+))/);
        const aa=amountMatch[0].split(' '); 
        const charge = aa[aa.length - 1].replace(/,/g,"");

        const taxPercentageMatch = rec.match(/\s+(\d+\.\d)+\s+(\d+\.\d)+\s+(\d+\.\d)+\s+/);
        const taxPercentage = taxPercentageMatch ? (parseFloat(taxPercentageMatch[2]) + parseFloat(taxPercentageMatch[3])) : "";
...
export const extracteWiderlogistics = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        const matchData = line.match(/\d+\s+\w+(\w|\s)+(\s|(\(+\s+(USD|INR)\s+.+\)))\s+\d+(,|.)(\d|\.\d)+\s+\d+(\s|\%)+(996|998|99)\d+/g,"") || [];

        if (matchData.length > 0) {
            // const description = matchData[0].match(/\w+\s+.+996+\d+/g, "");
            const descriptionMatch = line.match(/\d+\s+\w+(\w|\s)+(\s|(\(+\s+(USD|INR)\s+.+\)))\s+\d+(,|.)(\d|\.\d)+\s+\d+(\s|\%)+(996|998|99)\d+/g,"");
            const description =descriptionMatch && descriptionMatch[0] ? descriptionMatch[0].replace(/(\s|(\(+\s+(USD|INR)\s+.+\)))\s+\d+(,|.)(\d|\.\d)+\s+\d+(\s|\%)+(996|998|99)\d+/g, "").replace(/\d/g,"") : null;

            const HSN = line.match(/(996|998|99\d)+\d+/g, "");

            const unitQuantityMatch = line.match(/\(+\s+(USD|INR)\s+\d+/g,"");
            const unitQuantity = unitQuantityMatch && unitQuantityMatch[0] ? unitQuantityMatch[0].replace(/\(+\s+(USD|INR)\s/g, "") : null;

            const unitPriceMatch = line.match(/\(+\s+(USD|INR)\s+\d+\s+\w+\s+(INR|USD)\s+\d+(,|.)(\d|\.\d)+(\s|\))/g,"");
            const unitPrice =unitPriceMatch && unitPriceMatch[0] ? unitPriceMatch[0].replace(/\(+\s+(USD|INR)\s+\d+\s+\w+\s+(INR|USD)\s+/g,"").replace(/\)/g,"").replace(/\,/g,"") : null;

            const chargeMatch = line.match(/\d+(,|.)(\d|\.\d)+\s+\d+(\s|\%)+\s+(996|998)\d+/g, "");
            const charge = chargeMatch && chargeMatch[0] ? chargeMatch[0].replace(/\s+\d+(\s|\%)+\s+(996|998)\d+/g, "").replace(/,/g, "") : null;

            const taxPercentageMatch = line.match(/\d+(\s|\%)+\s+(996|998)\d+/g,"");
            const taxPercentage = taxPercentageMatch && taxPercentageMatch[0] ? taxPercentageMatch[0].replace(/\s+(996|998)\d+/g, "").replace(/%/g, "") : null;
    ...
    const invoiceDateRegex = /INVOICE+\s+DATE+\s+\:+\s+(\d+\-+\w+\-+\d+)/;
    const invoiceNumberRegex = /DS[A-Z0-9]{3}(\-)+(\d+)+/;
    const invoiceCurrency = "INR";
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
        ...
        if (extractedData && Array.isArray(extractedData)) {
            for (const line of extractedData) {
                ...
                if (gstMatch && !gstNumberExtracted) {
                    ...
                    const invoiceDateData = extractedData.find((item) =>item.content.match(invoiceDateRegex) );
                    invoiceDate = invoiceDateData ? invoiceDateData.content.replace(/INVOICE+\s+DATE+\s+\:+\s+/g, "") : "";

                    const invoiceNumberData = extractedData.find((item) => item.content.match(invoiceNumberRegex));
                    invoiceNumber = invoiceNumberData ? invoiceNumberData.content.replace(/(INVOICE)+\s+(NO)\s+\:\s+/g,"") : "";
...
export const extractedKwe = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (matchData.length > 0) {
            const descriptionMatch = line.match(/\w+\s+(\w|\s)+(\(|\s\w)+\d+(\s|\d)(\*|\@|\s)\s+\d+(,|\.\d)\d+.+/g,"");
            const description =descriptionMatch && descriptionMatch[0] ? descriptionMatch[0].replace(/(NO|YES|\(|\s\w)+\d+(\s|\d)(\*|\@|\s)\s+\d+(,|\.\d)\d+.+/g,"").replace(/\d/g,"") :"";
            ...
            // /(996|998)\d+\s+((\d+(,|\.\d|d)\d+\s+)|(\d+(,|\.\d|d)\d+\s+))+\d+(,|.)+(\.\d)\d+\s+(\d|(\d+(,|.)+(\.\d)\d))+/
           
            const chargeMatch = line.match(/((996|998)\d+\s+(\d+(\.\d)\d)\s+(\d+(\.\d)\d)\s+(\d+(,|.)(\d|\.\d)+)\s+)|((996|998)\d+\s+(\d+(\.\d)\d)+\s+(\d+(,|.)(\.\d|\d)+)\s+)/g, "");
            // const charge = chargeMatch && chargeMatch[0] ? chargeMatch[0].replace(/(((996|998)\d+\s+(\d+(\.\d)\d)+\s+))|((996|998)\d+\s+(\d+(\.\d)\d)\s+(\d+(\.\d)\d)\s+)/g,"").replace(/,/g, "") : null;
            
            const chargeArra = chargeMatch && chargeMatch[0] ? chargeMatch[0].replace(/,/g,"").split(chargeMatch[0].match(/(996|998)\d+/g, ""))[1].split(' ').filter(rec => rec != '') : [];
            console.log(chargeArra,'chargeArra','')
            const charge=chargeArra[0]==9?chargeArra[2]:chargeArra[1];
            ...
            const taxAmountMatch = line.match(/(996|998)\d+\s+((\d+(,|\.\d|d)\d+\s+)|(\d+(,|\.\d|d)\d+\s+))+\d+(,|.)+(\.\d)\d+\s+(\d|\d+(,|.)\d)+/g, "");
            const taxAmount = taxAmountMatch && taxAmountMatch[0] ? taxAmountMatch[0].replace(/(996|998)\d+\s+((\d+(,|\.\d|d)\d+\s+)|(\d+(,|\.\d|d)\d+\s+))+\d+(,|.)+(\.\d)\d+\s+/g, "").replace(/,/g, "") : "";
            // .replace(/((996|998)\d+\s+(\d+(\.\d)\d)\s+(\d+(\.\d)\d)\s+)|((996|998)\d+\s+(\d+(\.\d)\d)+\s)/g, "").
    ...
    const invoiceDateRegex = /\d+\s+(\/)+\s+\d+\s+(\/)+\s+\d+/;
    const invoiceNumberRegex = /\w+\s+\w+\s+(\:)+\s+(\d{14})/;
    const invoiceCurrency = "INR";
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        ...
        if (extractedData && Array.isArray(extractedData)) {
            for (const line of extractedData) {
                const gstMatch = line.content.match(/[0-9]+\s+[A-Z]+\s+\d+\s+[A-Z]+.+?(KWE)/g,"");
                if (gstMatch && !gstNumberExtracted) {
                    // if (!isAlreadyClientMatched) {
                    //     isAlreadyClientMatched = true;
                    //     continue;
                    // }
                    const gstNumber = gstMatch[0].replace(/KWE/g,"").trim().replace(/\s/g,"");

                    venName = "Kintetsu World Express (India) Pvt. Ltd.";

                    const invoiceDateData = extractedData.find((item) => item.content.match(invoiceDateRegex));
                    invoiceDate = invoiceDateData ? invoiceDateData.content.replace(/\s/g,""): "";

                    const invoiceNumberData = extractedData.find((item) =>item.content.match(invoiceNumberRegex));
                    invoiceNumber = invoiceNumberData ? invoiceNumberData.content.replace(/\w+\s+\w+\s+(\:)+\s+/g,"") : "";
        ...
        console.log("KWE PDF DATA", JSON.stringify(extractedData, null, 2));
        console.log("PDF HSN DATA", JSON.stringify(structuredHSNLines, null, 2));
        console.log("PDF INVOICE Data", JSON.stringify(InvoiceLines, null, 2));
    }
    return {
...
export const extractedWorldLine = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        // /(996|998)\d+\s+(\d+\.\d+)\s+(\d+(,|.)(\d|\.\d)+)\s+(\d+(,|.)(\d|\.\d)+)/ -- HSN line regex =  HSN  and Tax % & above = description charge , Tax Amount 
        // /(INR|USD)\s+(\d+(,|.)(\.\d|\d)+)+\s+(\d+(,|.)(\.\d|\d)+)+\s+((CBM|KGS)|(\d+(,|.)(\.\d|\d)+))\s+((\d+(,|.)(\.\d|\d)+\s+\w)|\w)/ -- INR line regex = Unit price and quantity 
        if (matchData) {
    ...
    console.log("WORLDLINE PDF DATA", JSON.stringify(allLines, null, 2))
    console.log("PDF HSN DATA", JSON.stringify(structuredHSNLines, null, 2));
    console.log("PDF INVOICE Data", JSON.stringify(InvoiceLines, null, 2));
    return {
        extractedData: InvoiceLines[0],
...
//             const description = mainDataaParts.slice(0, -5).join(' ').replace(/\b(W\/M|Flat Rate)\b/g, "").replace(/\d/g,"");
...
//             // const unitQuantity = line.match(/(W\/M|Flat Rate)\s+(\d+(?:,\d{3})*(?:\.\d+)?)\s+(INR|USD)\s+(\d+(?:[,.]\d+)?)\b/g,"");
//             // .match(/(\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"")
//             // const unitPriceMatch= line.match(/(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"") ;
//             // const unitPrice = unitPriceMatch ? unitPriceMatch[0].replace(/(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s/g,"").replace(/(INR|USD)\s+(\d+\.\d+")/g,""):"";
//             // /(\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/ -- for description line
...
//                 const invoiceAmountData = extractedData.find((item) => item.content.match(invoiceAmountRegex));
//                 invoiceAmount = invoiceAmountData ? invoiceAmountData.content.replace(/INR+\s+/g, "").replace(/\,/g,"") : '';
...
//         const matchData = line.match(/(SAC|\w)\s+\w+\:+\s+(996|998)\d+\s+(IGST|CGST|SGST)\s+\d+(,|.)(\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/g,"");

//         if (matchData) {
//             const aaaa = matchData[0];
//             const HSN = matchData[0].replace(/(SAC|\w)\s+\w+\:+\s+/g, "").replace(/\s+(IGST|CGST|SGST)\s+\d+(,|.)(\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/g,"");
//             const charge = matchData[0].replace(/T+\s+\d+(,|\.\d)+\d+\s+/g, "").replace(/\d+(,|\.\d)(\d|\.\d)+\s+\d+(,|\.\d)(\d|\.\d)+\s+\d+(,|\.\d)(\d)+\s+\d+(,|\.\d)(\d|\.\d)+\s+996\d+/g, "").replace(/,/g, "");
//             const taxAmount = matchData[0].replace(/(IGST|CGST|SGST)\s+\d+(,|.)(\.\d)+\s+/g, "").replace(/,/g, "");
//             const taxPercentage = matchData[0].replace(/\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/g, "").replace(/\%/g,"");
//             const description = extractedData[hsnId + 1].content.replace(/\d+/g, "");
...
export const extractChRowbin = async (pdf) => {
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (line.match(/(\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"")) {
            const matchData = line.match(/(\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"") || [];
            const description =matchData && matchData[0] ? matchData[0].replace(/\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"") : null;
            const unitQuantity =matchData&& matchData[0] ? matchData[0].replace(/(\w|\s)+\s+(W\/M|Flat Rate)+\s/g, "").replace(/(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"") : '';
            const unitPriceMatch= line.match(/(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/);
            const unitPrice = unitPriceMatch[2] ? unitPriceMatch[2].replace(/\,/g,"") : null;
            const charge =unitPriceMatch[6] ? unitPriceMatch[6].replace(/\,/g,"") : null;
        ...
        // /(SAC|\w)\s+\w+\:+\s+(996|998)\d+\s+(IGST|CGST|SGST)\s+\d+(,|.)(\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/
        // /(SAC|\w)\s+\w+\:+\s+(996|998)\d+\s+(IGST|CGST|SGST)\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/
        if (line.match(/(SAC|\w)\s+\w+\:+\s+(996|998)\d+\s+(IGST|CGST|SGST)\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/g,"")) {
            const taxMatch = line.match(/(SAC|\w)\s+\w+\:+\s+(996|998)\d+\s+(IGST|CGST|SGST)\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/g,"") || [];
            const taxPercentageData = taxMatch[0] ? taxMatch[0]: '';
            const taxPercentage = taxPercentageData.replace(/(SAC|\w)\s+\w+\:+\s+(996|998)\d+\s/g,"").replace(/\s+(INR|USD)\s+\d+(,|.)(\d|\.\d)+\d/g,"").replace(/(CGST|IGST|SGST)/g,"").replace(/%/g, "");
            const taxAmount = taxMatch[0] ? taxMatch[0].replace(/(SAC|\w)\s+\w+\:+\s+(996|998)\d+\s+(IGST|CGST|SGST)\s+\d+(,|.)(\d|\.\d)+\s+/g,"").replace(/(INR|USD)/g,""): '';
            const HSN =  taxMatch[0] ? taxMatch[0].replace(/(SAC|\w)\s+\w+\:+\s+/g,"").replace(/\s+(IGST|CGST|SGST)\s+\d+(,|.)(\d|\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/g,"") : '';
    ...
    let venName = '';
    const invoiceDateRegex = /(January|February|March|April|May|June|July|August|September|October|November|December)\s\d{1,2},\s\d{4}/;
    const invoiceNumberRegex =/190+[A-Z0-9]+\d[A-Z0-9]+/;
    const invoiceCurrency = 'INR';
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            const gstMatch = line.content.match(/(GST|[A-Z])+\s+[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/g,"");
            if (gstMatch && !gstNumberExtracted) {
                venName = 'C.H.ROBINSON Worldwide Freight India Pvt. Ltd.';
                const gstNumber = gstMatch[0].replace(/GST/g,"");  
                let invoiceNumber = '';
                let invoiceDate = '';
...
//     for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
//         const line = extractedData[hsnId].content;
//         // /((\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+))|((IGST|CGST|SGST)\s+\d+(,|.)(\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d)/
//         const matchData = line.match(/(\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/) || [];

//         if (matchData.length > 0) {
//             const descriptionMatch = line.match(/(\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g);
//             const description =descriptionMatch && descriptionMatch[0] ? descriptionMatch[0].replace(/\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,""): null;

//             const HSN = line.match(/SAC\s(\w+:)+\s+(\d+)\s+(\w+)\s+(\d.+)/g, "");

//             const unitQuantityMatch = line.match(/(\w|\s)+\s+(W\/M|Flat Rate)+\s+((\d+(,|\.\d)\d+)|(\d))+\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"");
//             const unitQuantity = unitQuantityMatch ? unitQuantityMatch[0].replace(/(\w|\s)+\s+(W\/M|Flat Rate)+\s/g, "").replace(/(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/g,"") : null;

//             const unitPriceMatch = line.match(/(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)\s+(INR|USD)+\s+(\d+(,|.)(\d|\.\d)+)/);
//             const unitPrice =unitPriceMatch && unitPriceMatch[2] ? unitPriceMatch[2].replace(/\,/g,"") : null;
//             const charge = unitPriceMatch && unitPriceMatch[6] ? unitPriceMatch[6].replace(/,/g, "") : null;

//             const taxPercentageMatch = line.match(/(IGST|CGST|SGST)\s+\d+\.\d+/g, "");
//             const taxPercentage = taxPercentageMatch && taxPercentageMatch[0] ? taxPercentageMatch[0].replace(/(CGST|IGST|SGST)\s+/g, "").replace(/%/g, "") : null;

//             const taxAmountMatch = line.match(/(IGST|CGST|SGST)\s+\d+(,|.)(\.\d)+\s+(INR|USD)\s+\d+(,|.)(\.\d)+\d/g, "");
//             const taxAmount = taxAmountMatch && taxAmountMatch[0] ? taxAmountMatch[0].replace(/(IGST|CGST|SGST)\s+\d+(,|.)(\.\d)+\s+(INR|USD)\s+/g, "").replace(/,/g, "") : "";
...
export const extractDBSCHENKAR = async (pdf) => {
    ...
    const invoiceDateRegex = /\d{1,2} [A-Z][a-z]{2} \d{4}/;
    const invoiceNumberRegex = /(103|102|10)\d{9}/;
    const invoiceCurrency = 'INR';
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
...
export const extractCeva = async (pdf) => {
    ...
    const currencyRegex = /SAC\s+Code\s+Curr.+/;
    const currencyData = extractedData.find((item) => item.content.match(currencyRegex));
    currency = currencyData ? currencyData.content : '';
...
export const extractedunitedLiner = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
        ...
        if (extractedData && Array.isArray(extractedData)) {
            for (const line of extractedData) {
                ...
                if (gstMatch && !gstNumberExtracted) {
                    ...
                    const invoiceAmountData = extractedData.find((item) => item.content.includes("Bankers:"));
                    if (invoiceAmountData) {
                        const previousItemIndex = extractedData.indexOf(invoiceAmountData) - 1;
                        if (previousItemIndex >= 0 && extractedData[previousItemIndex].content.trim().match(/\d+(,|.)(\d|\.\d)+/)) {
                            invoiceAmount = extractedData[previousItemIndex].content.trim().replace(/,/g,"");
                        }
                    }

                    const igstData = extractedData.find((item) => item.content.includes("Bankers:"));
                    if (igstData) {
                        const previousItemIndex = extractedData.indexOf(igstData) - 2;
                        if (previousItemIndex >= 0) {
                            const igstValues = extractedData[previousItemIndex].content.match(/\d+(,|.)(\d|\.\d)+/g);
                            if (igstValues && igstValues.length >= 2) {
                                igst = igstValues[1].replace(/,/g, "");
                            }
                        }
                    }
                    
                  
                    // const cgstDta = extractedData.find((item) => item.content.includes("Total : "));
                    // if (cgstDta) {
                    //     const previousItemIndex = extractedData.indexOf(cgstDta) + 1;
                    //     if (previousItemIndex >= 0) {
                    //         const previousItemContent = extractedData[previousItemIndex].content;
                    //         const match = previousItemContent.match(/\d+(,|.)(\d|\.\d+)/);
                    //         if (match) {
                    //             cgst = match ? match[0].replace(',', '') : "";
                    //         }
                    //     }
                    // }
                    
                    // const sgstData = extractedData.find((item) => item.content.includes("Total : "));
                    // if (sgstData) {
                    //     const previousItemIndex = extractedData.indexOf(sgstData) + 1;
                    //     if (previousItemIndex >= 0) {
                    //         const previousItemContent = extractedData[previousItemIndex].content;
                    //         const match = previousItemContent.match(/\d+(,|.)(\d|\.\d+)/);
                    //         if (match) {
...
export const extractedwenParker = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
        ...
        if (extractedData && Array.isArray(extractedData)) {
            for (const line of extractedData) {
                ...
                if (gstMatch && !gstNumberExtracted) {
                    ...
                    // const igstData = extractedData.find((item) => item.content.includes("Sub Total(s)"));
                    // if (igstData) {
                    //     const previousItemIndex = extractedData.indexOf(igstData) + 1;
                    //     if (previousItemIndex >= 0 && extractedData[previousItemIndex].content.trim().match(/^\d+(\.\d+)?$/)) {
                    //         igst = extractedData[previousItemIndex].content.trim();
                    //     }
                    // }
                    const cgstData = extractedData.find((item) => item.content.includes("Sub Total(s)"));
                    if (cgstData) {
                        const previousItemIndex = extractedData.indexOf(cgstData) + 1;
                        if (previousItemIndex >= 0 && extractedData[previousItemIndex].content.trim().match(/^\d+(\.\d+)?$/)) {
                            cgst = extractedData[previousItemIndex].content.trim();
                        }
                    }
                    const sgstData = extractedData.find((item) => item.content.includes("Sub Total(s)"));
                    if (sgstData) {
                        const previousItemIndex = extractedData.indexOf(sgstData) + 1;
                        if (previousItemIndex >= 0 && extractedData[previousItemIndex].content.trim().match(/^\d+(\.\d+)?$/)) {
                            sgst = extractedData[previousItemIndex].content.trim();
                        }
                    }
                    const currentInvoice = {
                        venName: venName,
...
export const extractedFederationofIndian = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        const line = extractedData[hsnId].content;
        if (line.match(/(\d+)\s+(\d+(\.\d+)?)\s+(\d+(\.\d+)?)\s+(\d+)%\s+(\d+(\.\d+)?)/)) {  
            const matchData = line.match(/(\d+)\s+(\d+(\.\d+)?)\s+(\d+(\.\d+)?)\s+(\d+)%\s+(\d+(\.\d+)?)/) || [];  
            const HSN = matchData[1] || " ";  
            const taxAmount = matchData[2] || 0;  
            const charge = matchData[7] || 0;  
            const taxPercentage = matchData[6] || 0;  
            ...
            withOutDuplicates.push(currentHSN)
        }
        if (line.match(/\d+\s+\w+(\s|.)(-|.|\w|\s)(\s|.|\w)(\w+)(.|\s)(\s|\w+)\s+\d+(,|.|\d|\s)\d+/)) { 
            const taxMatch = line.match(/\d+\s+\w+(\s|.)(-|.|\w|\s)(\s|.|\w)(\w+)(.|\s)(\s|\w+)\s+\d+(,|.|\d|\s)\d+/) || []; 
            const description = taxMatch[0] ? taxMatch[0].trim().replace(/\d/g, "") : " "; 
            currentTax.push(description)

        }
    };
    if (currentTax.length) {
        for (let i = 0; i < currentTax.length; i++) {
            withOutDuplicates[i].description = currentTax[i];
        }
    }
    const k = withOutDuplicates.map((rec) => {
        return JSON.stringify(rec)
    })
    const y = [...new Set(k)].forEach((rec) => structuredHSNLines.push(JSON.parse(rec)))
    ...
    let venName = '';
    const invoiceDateRegex = /\d+-\w+-\d+/;
    const invoiceNumberRegex = /(FIO)[A-Z0-9]{13}/; 
    const invoiceCurrency = 'INR';
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            const gstMatch = line.content.match(/[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/g);
            if (gstMatch && !gstNumberExtracted) { 
                venName = 'Federation of Indian Export Organisations'; 
                const gstNumber = gstMatch[0]; 
                let invoiceNumber = ''; 
                let invoiceDate = ''; 
                const invoiceAmount = structuredHSNLines.reduce((add, hsnLine) => {
                ...
                const invoiceDateData = extractedData.filter((item) => item.content.match(invoiceDateRegex)); 
                invoiceDate = invoiceDateData ? invoiceDateData[1].content.replace(/FIO\w{13}\s+dt+.\s+/g, "").trim() : '';
    ...
    console.log("FEDERATION OF INDIA", JSON.stringify(allLines, null, 2))
    return {
        extractedData: InvoiceLines[0],
        extractedHsnData: structuredHSNLines
    }
...
export const extractedGateway = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    console.log("GATEWAY PDF DATA", JSON.stringify(allLines, null, 2))
    console.log("PDF HSN DATA", JSON.stringify(structuredHSNLines, null, 2));
    console.log("PDF INVOICE Data", JSON.stringify(InvoiceLines, null, 2));
    return {
        extractedData: InvoiceLines[0],
...
export const extractedHellmann = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (extractedData[hsnId] && extractedData[hsnId].content) {
            ...
            if (matchData.length > 0) {
                ...
                const mainMatchDesc = line.match(/(.*?)(\d+)\s+([\d.]+)\s+([\d.]+%)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);
                description = mainMatchDesc ? (mainMatchDesc[1] || "") : "";

                if (previousAmountIndex !== -1 && !description) {
                    const amountIndex = extractedData.slice(previousAmountIndex + 1, hsnId).map(data => data.content);
                    description = amountIndex.join(' ');
                } else if (!description) {
                    description = extractedData.slice(extractedData.indexOf(totalData) + 1, hsnId).map((item) => item.content).join('\n').replace(/\d+/g, "");
    ...
    console.log("HELLMANN PDF DATA", JSON.stringify(allLines, null, 2))
    return {
        extractedData: InvoiceLines[0],
        extractedHsnData: structuredHSNLines
    }
...
export const extractedJeena = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        const matchData = line.match(/(\d.+)\s+(\d+)\s+([\d,.]+)\s+([\d,.]+)\s+(\d+\.\d+%)?\s+([\d,.]+)\s+(\d+\.\d+%.)?\s+([\d,.]+(\d.+))?/) || [];
        ...
        if (matchData.length > 0) {
            ...
            const taxPercentage = (matchData[7] || matchData[8]?.split(" ")[3]).replace(/.0%/g, "");
           
            const taxAmount = (matchData[8]?.replace(/0.00\s+18.0%/, '').trim() || '')
    ...
    let venName = '';
    const invoiceDateRegex = /DATE\s*:\s*(\d{1,2}\/\d{1,2}\/\d{4})/
    const invoiceNumberRegex = /IVJ[A-Z0-9]{13}/;
    const invoiceAmountRegex = /TOTAL INVOICE AMOUNT\s*₹\s*([\d,.]+)/
    const igstRegex = /GRAND TOTAL\s*([\d,.]+)\s+([\d,.]+)\s+\d+(,|.)\d+(,|.)\d+/
    const cgstRegex = /GRAND TOTAL\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)/
    const sgstRegex = /GRAND TOTAL\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)/
    const invoiceCurrency = 'INR';
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            ...
            if (gstMatch && !gstNumberExtracted) {
                ...
                const invoiceAmountData = extractedData.find((item) => item.content.match(invoiceAmountRegex));
                invoiceAmount = invoiceAmountData ? invoiceAmountData.content.replace(/ TOTAL INVOICE AMOUNT   ₹ /g, "") : '';

                const invoiceDateData = extractedData.find((item) => item.content.match(invoiceDateRegex));
                invoiceDate = invoiceDateData ? invoiceDateData.content.replace(/INV\s*NO\s*:\s*/g, '').replace(/DATE\s*:\s*/g, '').replace(/IVJ[A-Z0-9]{13}/g, '').trim() : '';
                ...
                const igstData = extractedData.find((item) => item.content.match(igstRegex));
                igst = igstData ? igstData.content.match(igstRegex)[0].split(' ')[10] : '0.00';

                const cgstData = extractedData.find((item) => item.content.match(cgstRegex));
                cgst = cgstData ? cgstData.content.match(cgstRegex)[4] : "0.00"
               

                const sgstData = extractedData.find((item) => item.content.match(sgstRegex));
                sgst = sgstData ? sgstData.content.match(sgstRegex)[4] : "0.00"
    ...
    console.log("JEENA & COMPANY PDF DATA", JSON.stringify(allLines, null, 2))
    return {
        extractedData: InvoiceLines[0],
        extractedHsnData: structuredHSNLines
    }
...
export const extractedKerryIndev = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        const matchData = line.match(/\d+(,|\d|.)\d+(.|\d|\s)\d+\s+\d+%\s+\d+(,|.)\d+(.|\d|\s)\d+\s+\d+.\d+\s+\d+(,|.|\d|\s)\d+(,|.|\d|\s)\d+\s+\d+(,|.|\d|\s)\d+\s+\d+(,|.|\d|\s)\d+(,|.|\d|\s)(\d|\s)+\w+\s+\d+\s+\w*\s+(,|&|)(\s|\w|\d)*(,|.|\d)\d+(,|.|\d|s)\d*(\s|\d)*/) || [];
        
        const a = matchData[0]?.split(" ").length - 2
        
        if (matchData.length > 0) {
            const splitData = matchData[0]?.split(" ");
            const description = `${splitData[9] || ''} ${splitData[10] || ''} ${splitData[11] || ''}`;
            const HSN = matchData[0]?.split(" ")[8] || '';
            const taxPercentage = (matchData[0]?.split(" ")[1] || '').replace(/%/g, "");
            const unitQuantity = (matchData[0]?.split(" ")[5] || '');
            const unitPrice = (matchData[0]?.split(" ")[6] || '');
            const charge = (matchData[0]?.split(" ")[2] || '')
            const taxAmount = (matchData[0]?.split(" ")[a] || '').replace(/,/g, "") || '';
            const amount = (matchData[0]?.split(" ")[0] || '')
    ...
    let venName = '';
    const invoiceDateRegex = /(\d+-+\w+-+\d+)/;
    const invoiceNumberRegex = /(\d{3}\s*-\s*\d{12})/;
    const invoiceAmountRegex = /Total\s+in\s+INR\s+(\d{1,3}(?:,\d{3})*(?:\.\d{2})\s+\d{1,3}(?:,\d{3})*(?:\.\d{2}))/
    const invoiceCurrency = 'INR';
    const currentYear = new Date().getFullYear();
    const nextYear = currentYear + 1;
    const financialYear = `${currentYear}-${nextYear}`;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            const gstMatch = line.content.match(/[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/g);
            if (gstMatch && !gstNumberExtracted) {
                venName = 'KERRY INDEV LOGISTICS PVT LTD';
                const gstNumber = gstMatch[0];
                let invoiceNumber = '';
                let invoiceDate = '';
                let invoiceAmount = '';
                ...
                const invoiceAmountData = extractedData.find((item) => item.content.match(invoiceAmountRegex));
                invoiceAmount = invoiceAmountData ? invoiceAmountData.content.replace(/(\d+\,\d+\.\d+)\s+\w+\s+\w+\s+\w+\s+\d+(\,\d+)\.+\d+\s+/g, "") : '';
    ...
    console.log("KERRY INDEV PDF DATA", JSON.stringify(allLines, null, 2))
    return {
        extractedData: InvoiceLines[0],
        extractedHsnData: structuredHSNLines
    }
...
export const extractedLogWin = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        if (matchData.length > 0) {
            ...
            const description = (matchData[0] || '').split('[')[1].trim().split(']')[1].trim().replace(/(IGST|CGST|SGST|GST)\s+\d+%=\d+(.|\d)\d+(.|\s)(\s+)\d.+/g, "")
            const HSN = matchData[2] || '';
            const taxPercentage = matchData[4].replace(/%/g, "");
            const taxAmount = matchData[5].replace(/,/g, "") || '';
            const charge = matchData[7] || '';
            const unitPrice = matchData[7] || '';
    ...
    let venName = '';
    const invoiceDateRegex = /\d{1,2}-[A-Z][a-z]{2}-\d{2}/;
    const invoiceNumberRegex = /MAA[A-Z0-9]{9}|BLR[A-Z0-9]{10}/;
    const invoiceAmountRegex = /TOTAL\s+INR\s+(\d+|.)+([\d])/
    const igstRegex = /ADD IGST\s+(\d+|.)+([\d])/
    const cgstRegex = /ADD CGST\s+(\d+|.)+([\d])/
    const sgstRegex = /ADD SGST\s+(\d+|.)+([\d])/
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            ...
            if (gstMatch && !gstNumberExtracted) {
                ...
                venName = 'Logwin Air & Ocean India Pvt. Ltd';
                const gstNumber = gstMatch[0];
                let invoiceNumber = '';
                let invoiceDate = '';
                let invoiceAmount = '';
    ...
    console.log("LOGWIN PDF DATA", JSON.stringify(allLines, null, 2))
    return {
        extractedData: InvoiceLines[0],
        extractedHsnData: structuredHSNLines
    }
...
export const extractedGeodis = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    const structuredHSNLines = [];
    let currentHSN = null;
    const taxRegMatch = /996[0-9]{3}/;
    const taxMatchElement = extractedData.find((item) => item.content.match(taxRegMatch));
    const HSN = taxMatchElement.content.match(taxRegMatch);

    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        const line = extractedData[hsnId].content;

        const matchData = line.match(/(\s.+)(IGST|CGST|SGST)\s+(\d+)%=([\d,.]+)\s+([\d,.]+)\s*(.*)/) || []; 

        if (matchData.length > 0) {
            const mainDataMatch = matchData[0] || ''; 
            const description = matchData[1].replace(/996[0-9]{3}/g, '') || ''; 
            const taxPercentage = parseInt(matchData[3]); 
            const taxAmount = parseFloat(matchData[4].replace(/,/g, '')); 
            const charge = matchData[5].replace(/,/g, '') || ''; 
    ...
    let venName = '';
    const invoiceDateRegex = /(\d+-+\w+-+\d+)/; 
    const invoiceNumberRegex = /GIN[A-Z0-9]{11}/; 
    const invoiceAmountRegex = /TOTAL\s+INR\s+(\d+|.)+([\d])/ 
    const igstRegex = /ADD IGST\s+(\d+|.)+([\d])/ 
    const cgstRegex = /ADD CGST\s+(\d+|.)+([\d])/ 
    const sgstRegex = /ADD SGST\s+(\d+|.)+([\d])/ 
    ...
    if (extractedData && Array.isArray(extractedData)) { 
        for (const line of extractedData) { 
            const gstMatch = line.content.match(/[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/g); 
            if (gstMatch && !gstNumberExtracted) { 
                venName = 'Geodis India Private Limited'; 
                const gstNumber = gstMatch[0]; 
                let invoiceNumber = ''; 
                let invoiceDate = ''; 
                let invoiceAmount = ''; 
                ...
                const invoiceNumberData = extractedData.find((item) => item.content.match(invoiceNumberRegex));
                invoiceNumber = invoiceNumberData ? invoiceNumberData.content.match(invoiceNumberRegex)[0].trim() : ''; 
    ...
    console.log("GEODIS PDF DATA", JSON.stringify(allLines, null, 2))
    return {
        extractedData: InvoiceLines[0],
        extractedHsnData: structuredHSNLines
    }
...
export const extractedJas = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    for (let hsnId = 0; hsnId < extractedData.length; hsnId++) {
        ...
        const matchData = line.match(/(.+)\s+(IGST|CGST|SGST)\s+(\d+)\%\s+((,|.|\d|\s)\d+(,|.|\d)\d.+)/) || [];
      
        if (matchData.length > 0) {
            const mainDataMatch = matchData[0] || '';
            const description = matchData[1].replace(/\[+SAC:\s+\d+\]/g, '');
            const taxPercentage = parseInt(matchData[3]);
            const charge = matchData[4].replace(/,/g, '');
            const taxAmount = parseFloat((charge * taxPercentage / 100).toFixed(2));
            let taxType, igst, cgst, sgst;
            if (taxPercentage == 9) {
                taxType = 'CGST & SGST';
                igst = 0;
    ...
    let venName = '';
    const invoiceDateRegex = /INVOICE\s+DATE/;
    const invoiceNumberRegex = /MAA[A-Z0-9]{9}/;
    const invoiceAmountRegex = /TOTAL\s+INR\s+(\d+|.)+([\d])/
    const igstRegex = /ADD IGST\s+(\d+|.)+([\d])/
    const cgstRegex = /ADD CGST\s+(\d+|.)+([\d])/
    const sgstRegex = /ADD SGST\s+(\d+|.)+([\d])/
    ...
    if (extractedData && Array.isArray(extractedData)) {
        for (const line of extractedData) {
            const gstMatch = line.content.match(/[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[A-Z0-9]{1}[A-Z]{1}[A-Z0-9]{1}/g);
            if (gstMatch && !gstNumberExtracted) {
                venName = 'JAS FORWARDING WORLDWIDE PVT LTD';
                const gstNumber = gstMatch[0];
                let invoiceNumber = '';
                let invoiceDate = '';
                let invoiceAmount = '';
    ...
    console.log("JAS PDF DATA", JSON.stringify(allLines, null, 2))
    return {
        extractedData: InvoiceLines[0],
        extractedHsnData: structuredHSNLines
    }
...
export const extractedGLOBELINK = async (pdf) => {
    const allLines = await extractPDFDataToLinesData(pdf);
    const extractedData = allLines;
    ...
    if (extractedData && Array.isArray(extractedData)) {
        ...
        console.log("GLOBELINK PDF DATA", JSON.stringify(extractedData, null, 2));
        console.log("PDF HSN DATA", JSON.stringify(structuredHSNLines, null, 2));
        console.log("PDF INVOICE Data", JSON.stringify(InvoiceLines, null, 2));
    }
    return {
